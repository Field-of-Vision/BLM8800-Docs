...
MBED_NOINLINE void print_context_info(void)
{
    ...
    if (CFSR_reg.HALFWORD_UFSR) {
        ...
        if (CFSR_reg.UFSR.UNDEFINSTR) {
            ...
            uint32_t PC_adr1;
            if ((PC_adr0 >> 24) == (AIC_CACHE0_MEM_BASE >> 24)) {
                PC_adr1 = PC_adr0 - (AIC_CACHE0_MEM_BASE - AIC_FLASH_MEM_BASE);
            } else if ((PC_adr0 >> 24) == (AIC_CACHE1_MEM_BASE >> 24)) {
                PC_adr1 = PC_adr0 - (AIC_CACHE1_MEM_BASE - AIC_PSRAM_MEM_BASE);
            } else {
                PC_adr1 = PC_adr0;
            }
