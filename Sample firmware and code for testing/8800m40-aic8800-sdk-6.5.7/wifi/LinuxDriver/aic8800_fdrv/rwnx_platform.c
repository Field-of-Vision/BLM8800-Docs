...
nvram_info_t nvram_info = {
    ...
    .fixed_rate = {
        .enable       = 0,
        .rate_idx     = 16, // 16:mcs0_20m, 36:mcs5_20m
    },
};
...
{
    fixed_rate->enable       = nvram_info.fixed_rate.enable;
    fixed_rate->rate_idx     = nvram_info.fixed_rate.rate_idx;
    printk("%s:enable      :%d\r\n", __func__, fixed_rate->enable);
    printk("%s:rate_idx    :%d\r\n", __func__, fixed_rate->rate_idx);
}
...
{
    nvram_info.fixed_rate.enable    = fixed_rate->enable;
    nvram_info.fixed_rate.rate_idx  = fixed_rate->rate_idx;
    printk("%s:enable      :%d\r\n", __func__, fixed_rate->enable);
    printk("%s:rate_idx    :%d\r\n", __func__, fixed_rate->rate_idx);
    if (fixed_rate->enable) {
        int fixed_rate_idx = fixed_rate->rate_idx;
        if ((fixed_rate_idx >= 0) && (fixed_rate_idx < (N_CCK + N_OFDM + N_HT + N_VHT + N_HE_SU))) {
            ...
            printk("rc_set_rate, idx: %d, cfg:%x\r\n", fixed_rate_idx, (u16)rate_config.value);
            if ((error = rwnx_send_me_rc_set_rate(rwnx_hw, 0, //sta->sta_idx
                (u16)rate_config.value)) != 0) {
                printk("rc_set_rate error: %d\r\n", error);
            }
        }
    }
}
...
void rwnx_plat_nvram_set_value(char *command, char *value)
{
    ...
    } else if (!strcmp(command, "rate_enable")) {
        nvram_info.fixed_rate.enable = rwnx_atoi(value);
    } else if (!strcmp(command, "rate_idx")) {
        nvram_info.fixed_rate.rate_idx = rwnx_atoi(value);
    } else {
