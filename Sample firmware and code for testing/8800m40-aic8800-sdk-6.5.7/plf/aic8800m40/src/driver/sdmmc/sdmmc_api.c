...
#define WORKING_CLK_FREQ            (40000000)
#endif
...
static int mmc_read_multi_block(sdmmc_t *sdmmc, unsigned char *buff_rd, unsigned int block_start, unsigned int block_count)
{
    ...
    dma_ch_tbl0cr_set(ch, ((4 * DATARD_TRIG_TH) | (REQ_TBL0 << DMA_CH_RQTYP_LSB) | (AHB_WORD << DMA_CH_DBUSU_LSB) |
...
static int mmc_write_multi_block(sdmmc_t *sdmmc, unsigned char *buff_wr, unsigned int block_start, unsigned int block_count)
{
    ...
    dma_ch_tbl0cr_set(ch, ((4 * DATAWR_TRIG_TH) | (REQ_TBL0 << DMA_CH_RQTYP_LSB) | (AHB_WORD << DMA_CH_DBUSU_LSB) |
    ...
    // RESERVED2 -> last cmdidx, for CMD12 is sent by HW
    if ((AIC_SDMMC->RESERVED2 == cmd.cmdidx) && ((card_status & (R1_READY_FOR_DATA | R1_CUR_STATE(R1_STATE_MASK))) !=
        (R1_READY_FOR_DATA | R1_CUR_STATE(R1_STATE_TRAN)))) {
        mmc_host_trans_stop();
        return -MMC_ERR_RSPUNEXP;
    }
...
void sdmmc_init(void)
{
    ...
    #endif
    // iomux for sdmmc
    iomux_gpioa_config_sel_setf(10, 6); // d2
    iomux_gpioa_config_sel_setf(11, 6); // d3
    iomux_gpioa_config_sel_setf(12, 6); // cmd
    iomux_gpioa_config_sel_setf(13, 6); // clk
    iomux_gpioa_config_sel_setf(14, 6); // d0
    iomux_gpioa_config_sel_setf(15, 6); // d1
}
