...
#define ESP_XMODEM_HANDLE_TASK_SIZE 512
#define ESP_XMODEM_HANDLE_TASK_PRIO 12

static void esp_xmodem_stop_task(void *pvParameters)
{
    ESP_LOGE(TAG, "Enter esp_xmodem_stop_task");
    rtos_task_suspend(500);
    rtos_task_handle process_handle = (rtos_task_handle) pvParameters;
    if (process_handle) {
        rtos_task_delete(process_handle);
        process_handle = NULL;
    }
...
esp_xmodem_handle_t esp_xmodem_init(const esp_xmodem_config_t *config, void *transport_config)
{
    ...
    handle->is_file_data = true;//false;
    handle->write_len = 0;
...
esp_err_t esp_xmodem_stop(esp_xmodem_t *handle)
{
    ...
    if (rtos_task_create(esp_xmodem_stop_task, "xmodem_stop", XMDM_STOP_TASK, ESP_XMODEM_HANDLE_TASK_SIZE, handle->process_handle, ESP_XMODEM_HANDLE_TASK_PRIO, NULL)) {
