...
#endif
#if ((PLF_AIC8800) && !defined(CFG_WIFI_RAM_VER))
#include "system.h"
#endif
...
static err_t net_if_output(struct netif *net_if, struct pbuf *p_buf)
{
    ...
    #ifndef CFG_WIFI_TX_TASK
    if (netif_is_up(net_if) && fhost_tx_start(net_if->state, p_buf, NULL, NULL) == 0)
    {
        status = ERR_OK;
    }
    #else /* CFG_WIFI_TX_TASK */
    if (netif_is_up(net_if) && netif_send_to_tx_task(net_if->state, p_buf) == 0)
    {
        status = ERR_OK;
    }
    #endif /* CFG_WIFI_TX_TASK */
    #if 0
    else
    {
...
int net_if_is_up(net_if_t *net_if)
{
    return netif_is_up(net_if);
}
...
const ip4_addr_t *net_if_ip4_addr(net_if_t *net_if)
{
    return netif_ip4_addr(net_if);
}
...
net_buf_tx_t *net_buf_tx_alloc(const uint8_t *payload, uint32_t length)
{
    ...
    dma_push(&dma_desc, &dma_desc, DMA_CHANNEL_WIFI_DATA_TX);
    dma_lli_poll(DMA_CHANNEL_WIFI_DATA_TX);
    dma_ch_int_clear(DMA_CHANNEL_WIFI_DATA_TX);
    #endif
    return pbuf;
}
...
#if 1
static int lwip_sleep_is_allowed(void)
{
    return lwip_tpcbs_check_unsend();
}
#endif
...
#if 1
    sleep_entry_t lwip_entry = {
        NULL,
        lwip_sleep_is_allowed,
        NULL,
        NULL
    };
    sleep_entry_register(&lwip_entry);
#endif
...
static void net_l2_send_cfm(uint32_t frame_id, bool acknowledged, void *arg)
{
    ...
    #if ((PLF_AIC8800) && !defined(CFG_WIFI_RAM_VER))
    extern void fhost_sta_eapol_succtx_cnt(uint32_t frame_id, bool acknowledged);
    uint8_t chip_id = ChipIdGet(0);
    if (chip_id == 0x07) {
        fhost_sta_eapol_succtx_cnt(frame_id, acknowledged);
    }
    #endif
}
...
{
    struct l2_filter_tag *filter = NULL;
    struct mac_eth_hdr* ethhdr = pbuf->payload;
    uint16_t ethertype = ntohs(ethhdr->len);
    struct netconn *conn;
    struct netbuf *buf;
    int i;
