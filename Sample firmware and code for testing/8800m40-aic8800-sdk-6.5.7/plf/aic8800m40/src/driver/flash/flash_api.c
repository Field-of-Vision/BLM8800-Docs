...
{
    return ROM_CRC32Calc((unsigned char *)adr, len);
}
...
#define WIFI_INFO_AP_BLINK_MAGIC_FLAG_BIT   (0x01UL << 12)
...
int flash_wifi_info_read(unsigned int flag, void *buf, unsigned int len)
{
    ...
    if (wifi_info.magic_num == FLASH_WIFI_INFO_MAGIC_NUM) {
		...
		if (info_flag_bits & WIFI_INFO_AP_BLINK_MAGIC_FLAG_BIT) {
            memcpy((char *)buf + index, &wifi_info.blink_magic_num, sizeof(wifi_info.blink_magic_num));
            index += sizeof(wifi_info.blink_magic_num);
        }
...
void flash_wifi_info_write(unsigned int flag, const void *buf, unsigned int len)
{
	...
	if (flag & WIFI_INFO_AP_BLINK_MAGIC_FLAG_BIT) {
        memcpy(&wifi_info.blink_magic_num, (char *)buf + index, sizeof(wifi_info.blink_magic_num));
        index += sizeof(wifi_info.blink_magic_num);
    }
...
{
    return flash_wifi_info_read(WIFI_INFO_AP_BLINK_MAGIC_FLAG_BIT, (void *)addr, 1);
}
...
{
    flash_wifi_info_write(WIFI_INFO_AP_BLINK_MAGIC_FLAG_BIT, (void *)addr, 1);
}
