                              ...
                              void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t), uint8_t id)
{
    ...
    if (rtos_task_create((rtos_task_fct)USBH_Process_OS, "USBH task", USBH_TASK, USBH_PROCESS_STACK_SIZE,
                         phost, USBH_PROCESS_PRIO, &(phost->thread))) {
        USBH_ErrLog("Fail to create task");
        return USBH_FAIL;
    }
#endif
...
#if (USBH_USE_OS == 1)
    rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_PORT_EVENT), false);
#endif
    return USBH_OK;
}
...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
    ...
    uint8_t idx = 0;
    //dbg_test_print("%s %d\r\n", __func__, phost->gState);
    switch (phost->gState)
    {
        case HOST_IDLE :
            //dbg_test_print("%s is_connected %d\r\n", __func__, phost->device.is_connected);
...
#if (USBH_USE_OS == 1)
                rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_STATE_CHANGED_EVENT), false);
#endif
            }
        }
        break;
...
USBH_StatusTypeDef  USBH_LL_Connect(USBH_HandleTypeDef *phost)
{
    ...
    {
        dbg_test_print("%s %d HOST_DEV_ATTACHED\r\n", __func__, phost->gState);
        phost->gState = HOST_DEV_ATTACHED ;
    }
#if (USBH_USE_OS == 1)
    rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_PORT_EVENT), true);
#endif
    return USBH_OK;
}
...
{
    rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_URB_EVENT), true);
    return USBH_OK;
}
#endif
