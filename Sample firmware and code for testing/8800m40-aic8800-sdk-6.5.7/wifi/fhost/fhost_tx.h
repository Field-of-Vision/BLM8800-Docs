...
/// Macro used to lock the TX IPC resources mutex
#define FHOST_TX_IPC_RES_LOCK()     rtos_mutex_lock(fhost_tx_env.tx_ipc_res_lock, -1)
/// Macro used to unlock the TX IPC resources mutex
#define FHOST_TX_IPC_RES_UNLOCK()   rtos_mutex_unlock(fhost_tx_env.tx_ipc_res_lock)
...
int fhost_tx_direct(uint8_t *payload, uint32_t length);
