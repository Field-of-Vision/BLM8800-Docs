...
#define STDIO_UART_TOIRQEN_BIT  ((uint32_t)0x00000020)
#define STDIO_UART_TOIRQEN_POS  5
...
__INLINE void stdio_uart_irqctl_pack(uint8_t ptirqen, uint8_t toirqen, uint8_t msirqen, uint8_t lsirqen, uint8_t txirqen, uint8_t rxirqen)
{
    REG_PL_WR(STDIO_UART_IRQCTL_ADDR,  ((uint32_t)ptirqen << 7) | ((uint32_t)toirqen << 5) | ((uint32_t)msirqen << 3) | ((uint32_t)lsirqen << 2) | ((uint32_t)txirqen << 1) | ((uint32_t)rxirqen << 0));
}

__INLINE void stdio_uart_irqctl_unpack(uint8_t* ptirqen, uint8_t *toirqen, uint8_t* msirqen, uint8_t* lsirqen, uint8_t* txirqen, uint8_t* rxirqen)
{
...
__INLINE uint8_t stdio_uart_toirqen_getf(void)
{
    uint32_t localVal = REG_PL_RD(STDIO_UART_IRQCTL_ADDR);
    return ((localVal & ((uint32_t)0x00000020)) >> 5);
}

__INLINE void stdio_uart_toirqen_setf(uint8_t toirqen)
{
    REG_PL_WR(STDIO_UART_IRQCTL_ADDR, (REG_PL_RD(STDIO_UART_IRQCTL_ADDR) & ~((uint32_t)0x00000020)) | ((uint32_t)toirqen << 5));
}
...
// field definitions
#define STDIO_UART_RXTOTRIGEN_BIT   ((uint32_t)0x00040000)
#define STDIO_UART_RXTOTRIGEN_POS   18
...
#define STDIO_UART_RXTOTRIGEN_RST   0x0
#define STDIO_UART_TXTRIGTH_RST     0x10
#define STDIO_UART_RXTRIGTH_RST     0x10

__INLINE void stdio_uart_dbufth_pack(uint8_t rxtotrigen, uint8_t txtrigth, uint8_t rxtrigth)
{
    REG_PL_WR(STDIO_UART_DBUFTH_ADDR,  ((uint32_t)rxtotrigen << 18) | ((uint32_t)txtrigth << 9) | ((uint32_t)rxtrigth << 0));
}

__INLINE void stdio_uart_dbufth_unpack(uint8_t *rxtotrigen, uint8_t* txtrigth, uint8_t* rxtrigth)
{
...
__INLINE uint8_t stdio_uart_rxtotrigen_getf(void)
{
    uint32_t localVal = REG_PL_RD(STDIO_UART_DBUFTH_ADDR);
    return ((localVal & ((uint32_t)0x00040000)) >> 18);
}

__INLINE void stdio_uart_rxtotrigen_setf(uint8_t txtrigth)
{
    REG_PL_WR(STDIO_UART_DBUFTH_ADDR, (REG_PL_RD(STDIO_UART_DBUFTH_ADDR) & ~((uint32_t)0x00040000)) | ((uint32_t)txtrigth << 18));
}
