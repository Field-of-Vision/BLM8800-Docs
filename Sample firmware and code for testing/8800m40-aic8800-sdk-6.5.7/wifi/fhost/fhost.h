...
struct fhost_vif_tag
{
    ...
    uint8_t band;
    uint8_t chan_index;
    /// Judge whether the scan and connect progress are finished
    uint8_t scan_in_progress;
    uint8_t connect_in_progress;
    /// Judge whether happen eapol process
    uint8_t happen_eapol;
};
...
extern fhost_user_cfg_t fhost_usr_cfg;
...
extern void fhost_sta_netif_recover(void);
...
extern void ipc_app_hwinit(uint32_t ipc_irq_prio);
extern void restore_ipc_periph(void);
#endif
extern int fhost_ipc_cntrl_init(uint32_t ipc_irq_prio);
extern int ipc_host_cntrl_start(void);
extern void *get_vif_mgmt_sta_list(uint8_t fhost_vif_idx);
extern uint8_t vif_mgmt_sta_cnt(uint8_t fhost_vif_idx);
extern void ipc_host_wait_fw_ready(void);
extern int ipc_host_fw_init(void);
#if (PLF_AIC8800)
extern void aon_mailbox_restore(void);
#endif
...
#define USER_FILTER_MAX_NUM     8

/** VNET filter result */
enum VNET_FILTER_RESULT
{
    VNET_TRANSFER_MCU_HOST,
    VNET_TRANSFER_MCU_ONLY,
    VNET_TRANSFER_HOST_ONLY,
    VNET_TRANSFER_RESEND_ONLY,
};

typedef enum {
    PACKET_FILTER_MASK_SRC_IP         = 0x01,
    PACKET_FILTER_MASK_SRC_PORT       = 0x02,
    PACKET_FILTER_MASK_DST_PORT       = 0x04,
    PACKET_FILTER_MASK_PROTOCOL       = 0x08,
    PACKET_FILTER_MASK_SHARE_DNS_PORT = 0x10,
} IP_PACKET_FILTER_FIELD;
...
typedef struct ip_packet_filter
{
    ...
    uint16_t dst_port;     // Destination port, 67: DHCP Discover/Request(UDP), 68: DHCP Offer/Ack(UDP)
    uint16_t src_port;
    uint32_t src_ipaddr;   // Source IP addr
    uint8_t  filter_mask;
} ip_packet_filter_t;

typedef struct
{
    ip_packet_filter_t array[USER_FILTER_MAX_NUM];
    struct co_list used_list;
    rtos_mutex list_mutex;
} user_filter_t;

void set_hostif_wlan_status(hostif_status_e st);
hostif_status_e get_hostif_wlan_status(void);

extern user_filter_t user_filter;

/**
 * @brief: User set ip packet filter, max 8 filters supported
 * @param[in]   filter: pointer to the filter struct
 * @return      0 for success
 */
int set_hostif_user_filter(ip_packet_filter_t *filter);

/**
 * @brief: User clear ip packet filter that match protocol & dst_port
 * @return      0 for success, 1 for not found in used list
 */
int clear_hostif_user_filter(ip_packet_filter_t *filter);

/**
 * @brief: Init user filter
 */
void init_hostif_user_filter(void);
