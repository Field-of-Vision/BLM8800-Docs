...
python ../../../tools/scons.py -u . PRODUCT=host-wifi PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" \
        TGT_MODE=wifi_sntp CODE_START_ADDR=0x8004000 \
        SOFTAP=on PMIC_VER=lite HEAP_SIZE=0x16000 FHOST_APP=sntp_client TGTNAME=host_wb_sntp_client BAND5G=on SNTP=on OTA=on WIFI_RAM_VER=on $opt
