...
PRIVATE_HOST_DECLARE(G0RTOS, ticker_event_queue_t, events);
PRIVATE_HOST_DECLARE(G0RTOS, ticker_irq_handler_type, irq_handler);

void lp_ticker_data_save(void)
{
    backup_irq_handler = irq_handler;
    memcpy(&backup_events, &events, sizeof(events));
}
void lp_ticker_data_restore(void)
{
    irq_handler = backup_irq_handler;
    memcpy(&events, &backup_events, sizeof(events));
}
