 ...
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 *+                                                      Fs=48K          +
 *+                     RX01<---SRC<---EQ<---VOL_CTRL<---I2S0<---ADC0+1  +
 *+                                                            8/16/48K  +
 *++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#define AUD_MEM_WRITE         psi_write_op
#define AUD_MEM_READ          psi_read_op
#define AUD_MEM_MASK_WRITE    psi_mask_write_op
...
{
    uint8_t reset_pin_type = PIN_TYPE(audio_config_ext_aic1000_reset_pin_get());
    uint8_t reset_pin_idx = PIN_IDX(audio_config_ext_aic1000_reset_pin_get());
...
void aic1000_pd(void)
{
    ...
    // power off
    AUD_MEM_MASK_WRITE((unsigned int)(&aic1000audAnalogReg->sw_rst_ctrl),
        AIC1000AUD_ANALOG_REG_CFG_RTC_PWR_OFF_SEQ_EN, AIC1000AUD_ANALOG_REG_CFG_RTC_PWR_OFF_SEQ_EN);
}

void aic1000_iomux_config(void)
{
    AUD_MEM_WRITE((unsigned int)&aic1000audIomux->GPCFG[2], 2);
    AUD_MEM_WRITE((unsigned int)&aic1000audIomux->GPCFG[4], 2);
    AUD_MEM_WRITE((unsigned int)&aic1000audIomux->GPCFG[5], 2);

    AUD_MEM_MASK_WRITE((unsigned int)&aic1000audSysctrl->TPORTS_SEL,
        AIC1000AUD_SYS_CTRL_CFG_TOP_TPORTS_SEL(0x1E),
        AIC1000AUD_SYS_CTRL_CFG_TOP_TPORTS_SEL(0xFF));
    AUD_MEM_MASK_WRITE((unsigned int)&aic1000audSysctrl->TPORTS_SEL_2,
        ((0x07 << 6) |
        (0x06 << 12) |
        (0x05 << 15)),
        ((0x07 << 6) |
        (0x07 << 12) |
        (0x07 << 15)));
    AUD_MEM_MASK_WRITE((unsigned int)&aic1000audAudCodec->intf_ctrl,
        AIC1000AUD_AUD_CODEC_TPORTS_SEL(0x1),
        AIC1000AUD_AUD_CODEC_TPORTS_SEL(0x3F));

    // set spk/mic IO to Hi-Z
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[15]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[14]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[13]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[12]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[11]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[10]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[9]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[8]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[7]), 0x00010000);
    AUD_MEM_WRITE((unsigned int)&(aic1000audIomux->GPCFG[6]), 0x00010000);
}
...
void aic1000_clk_cfg(void)
{
...
#if PLF_AIC8800
    // function select debug_clk
    iomux_gpio_config_sel_setf(5, 6);
    // generate 26M debug_clk
    val = AIC_CPUSYSCTRL->TPSEL;
    val &= ~(0xFFUL << 8);
    val |= (0x18UL << 8);
    AIC_CPUSYSCTRL->TPSEL = val;
#elif PLF_AIC8800MC
    //TODO
    TRACE("Error: %s TODO\n", __func__);
#elif PLF_AIC8800M40
    ...
    //audio pll cfg0
    AUD_MEM_WRITE((unsigned int)&(aic1000audAudCtrlReg->aud_pll_cfg0),
    ...
    //audio pll cfg1
    AUD_MEM_WRITE((unsigned int)&(aic1000audAudCtrlReg->aud_pll_cfg1),
    ...
    //pu audio pll
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCtrlReg->aud_pll_cfg0),
    ...
    //while(pll_lock_ok==0)
    //{
    //    pll_lock_ok = ((AUD_MEM_READ((unsigned int)&(aic1000audAudCtrlReg->aud_pll_cfg0))) & AIC1000AUD_AUD_CTRL_REG_CFG_AUPLL_LOCK) == AIC1000AUD_AUD_CTRL_REG_CFG_AUPLL_LOCK;
    //}
    ...
    //audio aud_clk_ctrl0
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCtrlReg->aud_clk_ctrl0),
    ...
    AUD_MEM_WRITE((unsigned int)&(aic1000audAudCodec->iis_ctrl0),
    ...
    AUD_MEM_WRITE((unsigned int)&(aic1000audWdg->WDG_CTRL), 0x00000000);
}

void aic1000_ana_cfg(void)
{
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->ana_clk1),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_ivref),
        (0                                               |
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_micpga_1),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_micpga_2),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_micpga_3),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_micbias),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_sdm_adc0),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_sdm_dac1),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_hp_l),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->abb_hp_r),
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->spk_gain),
...
void aic1000_mic_matix_config(uint8_t type)
{
    ...
    //@ cfg adc path
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->core_ctrl1),
...
void aic1000_adc_pu(uint8_t ch_map)
{
    ...
    if (adc_ch_ana_pu == 0) {
        //@ clear adc mem
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl),
            AIC1000AUD_AUD_CODEC_MIC_MEM_CLKSEL,
            AIC1000AUD_AUD_CODEC_MIC_MEM_CLKSEL);
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl),
            AIC1000AUD_AUD_CODEC_MIC_MEM_CLR,
            AIC1000AUD_AUD_CODEC_MIC_MEM_CLR);
        while(!((AUD_MEM_READ((unsigned int)&(aic1000audAudCodec->mem_ctrl))) & AIC1000AUD_AUD_CODEC_MIC_MEM_CLR_DONE));
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl), 0, AIC1000AUD_AUD_CODEC_MIC_MEM_CLR);
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl), 0, AIC1000AUD_AUD_CODEC_MIC_MEM_CLKSEL);
    }
...
void aic1000_dac_pu(uint8_t ch_map)
{
    ...
    if (dac_ch_ana_pu == 0) {
        ...
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl),
            ...
            AIC1000AUD_AUD_CODEC_SPK_MEM_CLR);
        while(!((AUD_MEM_READ((unsigned int)&(aic1000audAudCodec->mem_ctrl))) & AIC1000AUD_AUD_CODEC_SPK_MEM_CLR_DONE));
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl), 0, AIC1000AUD_AUD_CODEC_SPK_MEM_CLR);
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mem_ctrl), 0, AIC1000AUD_AUD_CODEC_SPK_MEM_CLKSEL);
    }
...
void aic1000_adc_config(aic1000_cfg_t *cfg)
{
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->core_ctrl0),
    ...
    if ((cfg->ch_map & (AUD_CH_MAP_CH_0 | AUD_CH_MAP_CH_1)) != 0) {
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->iis_ctrl1),
            AIC1000AUD_AUD_CODEC_IIS0_ODAT_SEL(2),
    ...
    if ((cfg->ch_map & AUD_CH_MAP_CH_2) != 0) {
        ...
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->iis_ctrl0),
...
void aic1000_dac_config(aic1000_cfg_t *cfg)
{
    ...
    if ((cfg->ch_num == AUD_CH_NUM_1) && (cfg->ch_map == AUD_CH_MAP_CH_1)) {
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->dac_sdm_ctrl),
...
void aic1000_adc_start(aic1000_cfg_t *cfg)
{
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->iis_ctrl0),
        ...
        AIC1000AUD_AUD_CODEC_IIS_EN);
    // force bck enable
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->clk_ctrl),
    ...
    //@ check status
#if (AIC1000AUD_CHECK_STATUS)
    status = AUD_MEM_READ((unsigned int)&(aic1000audAudCodec->aud_cfg_sync));
...
void aic1000_dac_start(aic1000_cfg_t *cfg)
{
    ...
    if ((adc_ch_en & (AUD_CH_MAP_CH_0 | AUD_CH_MAP_CH_1)) == 0) {
        i2s0_trig_sel = 1;
    }
...
void aic1000_adc_stop(aic1000_cfg_t *cfg)
{
    ...
    if ((i2s0_en == 0) && (i2s1_en == 0)) {
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->iis_ctrl0), 0, AIC1000AUD_AUD_CODEC_IIS_EN);
        // force bck disable
        AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->clk_ctrl),
...
void aic1000_adc_dig_gain_config(uint8_t ch_map, uint8_t gain)
{
    ...
    AUD_MEM_MASK_WRITE((unsigned int)&(aic1000audAudCodec->mic_gain),
