...
uint32_t sw_rec_lpticker_irqstatus = 0;
...
/// Match registr high-16bit ignored
#define LPT_MATCH_HI_IGN_BIT    (0x01UL << (4 + LP_TICKER_TIMER_MRi))
/// Match enable
...
void lp_ticker_init()
{
    ...
    if (pwrctrl_pwrmd_cpusys_sw_record_getf() == CPU_SYS_POWER_ON_RESET) {
        ...
        if ((reg & 0x01UL) == 0x00UL) { // Init if disabled
            ...
            reg &= ~(0x03UL << 8);
            reg |=  (0x01UL | (tick_sel << 8)); // enable timer, set tick freq
            LP_TICKER_TIMER->CTL = reg;
        }
        if ((reg & LPT_MATCH_ENA_BIT) == 0x00UL) {
            LP_TICKER_TIMER->CTL = reg | LPT_MATCH_ENA_BIT | LPT_MATCH_HI_IGN_BIT;  // enable match, ignore MR high-16bit
        }
    }
...
{
    if (ignore) {
        LP_TICKER_TIMER->CTL |=  LPT_MATCH_HI_IGN_BIT;
    } else {
        LP_TICKER_TIMER->CTL &= ~LPT_MATCH_HI_IGN_BIT;
    }
}
...
void lp_ticker_awake_state_update(void)
{
    uint32_t status = LP_TICKER_TIMER->IS;
    sw_rec_lpticker_irqstatus = status;
    // wrap interrupt
    if (status & LPT_WINT_ST_MSK_BIT) {
        LP_TICKER_TIMER->IC = LPT_WINT_CLR_BIT; // clear interrupt
    }
...
uint32_t lp_ticker_sw_rec_irqstatus_get(void)
{
    return sw_rec_lpticker_irqstatus;
}

bool lp_ticker_awake_from_wrap_int(void)
{
    return (sw_rec_lpticker_irqstatus & LPT_WINT_ST_MSK_BIT) ? true : false;
}
