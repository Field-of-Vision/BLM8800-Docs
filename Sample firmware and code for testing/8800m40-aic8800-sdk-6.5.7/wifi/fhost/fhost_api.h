...
typedef struct {
    ...
    uint32_t ipc_e2a_msg_buf_num;
    uint8_t parse_rx_datarate_en; // enable parsing rx data rate
    uint8_t rf_switch_en; // enable rf switch
    uint8_t wifi_ram_ver_on;
#if (PLF_AIC8800)
    uint8_t fast_ram_on; // if shutdown fast memory
#endif
    uint8_t retransmit_via_app;
} fhost_user_cfg_t;

/// structure for super scan
struct fhost_super_scan_t {
    /// SSID to scan (NULL: no care)
    struct cfgrwnx_scan_ssid *ssids;
    /// Number of SSID in ssids, max 2 (0: no care)
    int ssid_cnt;
    /// Array of frequencies to scan or %NULL for all frequencies.
    /// The frequency is set in MHz. The array is zero-terminated.
    /// band MUST set -1
    int *freqs;
    /// Band(0:2G4, 1:5G, -1: no care)
    int band;
    /// BSSID to scan, can be NULL for wildcard BSSID
    const uint8_t *bssid;
    /// Extra IE to add in the proce request (NULL: no care)
    const uint8_t *extra_ies;
    /// Size of the extra IEs  (0: no care)
    int extra_ies_len;
};
...
int fhost_scan_gt_rssi(struct fhost_cntrl_link *link, int fvif_idx, int *freq_list, int8_t rssi);
/**
 ****************************************************************************************
 * @brief Scan Access Points, support setting
 *
 * E.g. struct fhost_super_scan_t param;
 *      struct cfgrwnx_scan_ssid ssid;
 *      uint8_t *ssid_str = "AP-Test";
 *      uint8_t bssid_str[] = {0x04, 0x03, 0x01, 0x05, 0x06, 0xc0};
 *      int freqs[] = {5180};
 ...
 *      param.freqs = freqs;
 *      fhost_super_scan(console_cntrl_link, fvif_idx, &param);
 *
 * @param[in] link      Pointer to a cfgrwnx link
 * @param[in] fvif_idx  Index of the interface
 *
 ...
 ****************************************************************************************
 */
int fhost_super_scan(struct fhost_cntrl_link *link, int fvif_idx, struct fhost_super_scan_t* param);
