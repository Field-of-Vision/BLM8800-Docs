...
#define core_util_critical_section_enter()  critical_section_start()
#define core_util_critical_section_exit()   critical_section_end()
#define MBED_ASSERT(...)                    do {} while (0)
...
void ticker_free(const ticker_data_t *const ticker)
{
    if (!ticker->queue->initialized) {
        return;
    }
    core_util_critical_section_enter();
    ticker->interface->free();
    ticker->queue->initialized = false;
    ticker->queue->suspended = false;
    core_util_critical_section_exit();
}
