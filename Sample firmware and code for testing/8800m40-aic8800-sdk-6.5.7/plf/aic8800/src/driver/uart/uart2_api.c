...
#define UART_INITED_CLR     M2STR_P3(ipccomreg_state_uart, UART2_INDEX, inited_clrb)
...
void uart2_init(void)
{
    ...
    NVIC_SetVector(UART2_IRQn, (uint32_t)uart2_isr);
...
void uart2_deinit(void)
{
    ...
    cpusysctrl_hclkmd_set(CSC_HCLKMD_UART2_DIS_BIT); // clk dis
    cpusysctrl_oclkmd_set(CSC_OCLKMD_UART2_DIS_BIT);
    uart2_inited = 0;
    UART_INITED_CLR();
}
...
void uart2_nputs(const char* str, int len)
{
    int idx = 0;
    for (; idx < len; idx++) {
        uart2_putc(str[idx]);
...
bool uart2_tstc(void)
{
    return uart2_dr_getf();
}
