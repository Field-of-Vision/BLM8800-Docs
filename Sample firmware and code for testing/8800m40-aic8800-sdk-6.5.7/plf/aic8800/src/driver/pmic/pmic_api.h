...
#define IS_PMIC_SRAM_VALID(addr)    (((unsigned int)(addr) >> 24) == (AIC_PMIC_RAM_BASE >> 24)) // 0x01000000 ~ 0x01FFFFFF
#define IS_PMIC_REG_VALID(addr)     (((unsigned int)(addr) >> 20) == (AIC_PMIC_REG_BASE >> 20)) // 0x50000000 ~ 0x500FFFFF

#define IS_PMIC_MEM_ADDR(addr)      (IS_PMIC_SRAM_VALID(addr) || IS_PMIC_REG_VALID(addr))
...
typedef enum {
    PMIC_POR_SRC_PB_INT_7S_SET      = (0x01 << 0),
    PMIC_POR_SRC_PB_INT_DEBC        = (0x01 << 1),
    PMIC_POR_SRC_BOR_PWR_ON         = (0x01 << 2),
    PMIC_POR_SRC_BOND_OPT_SET       = (0x01 << 3), // PMIC_VER=lite
    PMIC_POR_SRC_TWS_UART_PWR_SET   = (0x01 << 3), // PMIC_VER=aud
    PMIC_POR_SRC_RESEVERD0          = (0x01 << 4), // PMIC_VER=lite
    PMIC_POR_SRC_RTC_GPIO_PWR_ON    = (0x01 << 4), // PMIC_VER=aud
    PMIC_POR_SRC_ALM_INT_SET        = (0x01 << 5),
    PMIC_POR_SRC_WDG_PWR_ON         = (0x01 << 6),
} PMIC_POR_SRC_T;
...
/**
 * @brief: Get pwrkey holding state
 * @return: current holding state
 */
int pmic_powerkey_hold_get(void);
...
/**
 * @brief:      Config ldo avdd18
 * @param[in]   vbit: vbit_nor_tune
 */
void pmic_ldo_avdd18_config(uint32_t vbit);
