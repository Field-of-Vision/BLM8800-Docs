...
#define UART_INITED_CLR     M2STR_P3(ipccomreg_state_uart, UART1_INDEX, inited_clrb)
...
void uart1_init(void)
{
    ...
    if (iomux_agpio_config_sel_getf(15) == 0) {
        iomux_agpio_config_sel_setf(15, 2);
    }
    /* IOMUX UART1_RX = GPIO10    UART1_TX = GPIO11 */
    //iomux_gpio_config_sel_setf(10, 0x01);
    //iomux_gpio_config_sel_setf(11, 0x01);
    /* IOMUX UART1_RX = GPIO4    UART1_TX = GPIO5 */
    iomux_gpio_config_sel_setf(4, 0x03);
    iomux_gpio_config_sel_setf(5, 0x03);
    #else
...
void uart1_deinit(void)
{
    ...
    cpusysctrl_hclkmd_set(CSC_HCLKMD_UART1_DIS_BIT); // clk dis
    cpusysctrl_oclkmd_set(CSC_OCLKMD_UART1_DIS_BIT);
    uart1_inited = 0;
    UART_INITED_CLR();
}
...
void uart1_nputs(const char* str, int len)
{
    int idx = 0;
    for (; idx < len; idx++) {
        uart1_putc(str[idx]);
