...
python ../../../tools/scons.py -u . PRODUCT=host-bt PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" HW_PLATFORM=asic AON=on \
        TGT_MODE=bt \
        BT_AUDIO=on AUD_USED=off APPS=on LINK_ONE=on TGTNAME=bt_ota_box CONSOLE=on BT_OTA=on BT_OTA_BOX=on \
        AUDIO=on UTILS=on HW_SBC=on AAC=off PMIC_MCLK=on BT_PROMPT=off \
        ROM_VER=auto \
