...
typedef struct {
    patch_tbl_array_t array;
    uint32_t count;
} patch_array_t;

struct aic_patch_tag {
    #if (PLF_AIC8800MC)
    patch_ram_t ldpc_cfg;
    patch_ram_t agc_cfg;
    patch_ram_t txgain_map;
    #endif
    patch_array_t wifi_setting;
    patch_ram_t funcs;
    patch_array_t jump_tbl;
    patch_array_t others_tbl;
};

extern struct aic_patch_tag rom_patch_obj;
#endif

#define RWNX_DBG(fmt, ...)  do {} while(0) //dbg(fmt, ##__VA_ARGS__)
...
bool rwnx_ipc_a2e_tx_free_list_is_empty(void);
bool rwnx_ipc_a2e_tx_post_list_is_empty(void);
...
/**
 * @brief: check wifi fw active or not
 * @param[in] none
 * @return value: 1: active, 0: not active
 */
int wifi_fw_is_active(void);

/**
 * @brief: repower wifi fw, after deepsleep in disconneted state
 *         can't be called in isr
 * @param[in] none
 */
void wifi_fw_repower(void);
