...
int network_init(network_t *n, const char *host, const char *port, const char *ca, const char *cert, const char *key)
{
    if (NULL == n)
...
int network_set_ca(network_t *n, const char *ca, const char *cert, const char *key)
{
#ifndef MQTT_NETWORK_TYPE_NO_TLS
    if ((NULL == n) || (NULL == ca))
        RETURN_ERROR(MQTT_NULL_VALUE_ERROR);
