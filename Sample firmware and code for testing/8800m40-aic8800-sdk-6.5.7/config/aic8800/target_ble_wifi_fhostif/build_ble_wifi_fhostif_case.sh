...
# BLE_APP_HID         : off/on
python ../../../tools/scons.py -u . PRODUCT=host-wb PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" \
        TGT_MODE=ble_wifi_fhostif TGTNAME=host_wb_ble_wifi_fhostif \
        BT_BLE=on BT_AUDIO=off APPS=on BLE_ONLY=on BT_USER=on BLE_APP_HID=off \
        BLE_DFT_STATE=adv BLE_APP_AUDTRANSMIT=none BLE_APP_SMARTCONFIG=on \
        AUDIO=off UTILS=on HW_SBC=off AAC=off BT_PROMPT=off \
        ROM_VER=auto CHKSUM=off AON=on CRYPTO=mbedtls BAND5G=on HEAP_SIZE=0x20000 \
        SOFTAP=on PING=on IPERF=on FHOST_APP=fhostif HOSTIF=usb \
        PMIC_VER=lite $opt
