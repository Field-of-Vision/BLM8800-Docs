...
static int nettype_tls_init(network_t* n, nettype_tls_params_t* nettype_tls_params)
{
    ...
    if (NULL != n->client_cert) {
        if (0 != (rc = (mbedtls_x509_crt_parse(&(nettype_tls_params->client_cert), (unsigned char *)n->client_cert,
                                          (n->client_cert_len + 1))))) {
            MQTT_LOG_E("%s:%d %s()... parse client cert failed returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            RETURN_ERROR(rc);
        }
    }

    if (NULL != n->client_key) {
        if (0 != (rc = mbedtls_pk_parse_key(&(nettype_tls_params->private_key), (unsigned char *)n->client_key,
                                          (n->client_key_len + 1), NULL, 0 ))) {
            MQTT_LOG_E("%s:%d %s()... parse client key failed returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            RETURN_ERROR(rc);
        }
    }
    #endif
...
int nettype_tls_connect(network_t* n)
{
    ...
    #if defined(MQTT_TLS_LOG)
    mbedtls_debug_set_threshold(10);
    mbedtls_ssl_conf_dbg( &(nettype_tls_params->ssl_conf), dbg_test_print, NULL );
    #endif
