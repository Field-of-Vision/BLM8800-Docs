...
void gpadc_init(int gpbidx)
{
    ...
    if ((gpbidx == 2) || (gpbidx == 3)) {
        // mux en
        reg_val = AIC_ANAREG1->gpiob_ie & ~(ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_EN | ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_ENB);
        AIC_ANAREG1->gpiob_ie = reg_val | ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_EN | ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_ENB;
        // mux gpio
        gpiob_init(gpbidx);
        gpiob_dir_in(gpbidx);
...
void gpadc_free(int gpbidx)
{
    if ((gpbidx >= 2) && (gpbidx <= 3)) {
        ...
        // mux en
        reg_val = AIC_ANAREG1->gpiob_ie & ~(ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_EN|ANALOG_REG1_RF_CFG_ANA_GPIOB_MUX_ENB);
        AIC_ANAREG1->gpiob_ie = reg_val | 0;
        gpiob_deinit(gpbidx);
        gpiob_force_oen_disable(gpbidx);
    }
}
