...
/** @addtogroup USB_HOST_DRIVER
  * @{
  */

#if 1// defined (HAL_PCD_MODULE_ENABLED) || defined (HAL_HCD_MODULE_ENABLED)
...
static void USB_PhyInit(USB_OTG_GlobalTypeDef *USBx);
/* Exported functions --------------------------------------------------------*/
...
/**
  ...
  * @retval None
  */
void USB_Delay(uint32_t delay_ms)
{
    TIMER_INIT();
    uint32_t delta_us = delay_ms * 1000;
                               ...
                               USB_OTG_CfgTypeDef cfg)
{
    ...
    if (cpusysctrl_ulpics_get())
    {
        ...
        dbg_test_print("ULPI clk detected 0x%x\r\n", USBx);
        ...
        dbg_test_print("usb clk init done\r\n");
    }
    else
    {
        dbg_test_print("err!! no usb ulpi clk\r\n");
        return -1;
    }
    #else
     //add by jihuicui, clear bit14 in usb_cfg0
    AIC_MMSYSCTRL->usb_cfg0 &= ~(1<<14);
    ...
    for (int i = 0; i < cfg.Host_channels; i++)
    {
        USBx_HC(i)->HCINT = ~0;
    ...
    {
        USBx->GAHBCFG  = USB_OTG_GAHBCFG_DMAEN | USB_OTG_GAHBCFG_HBSTLEN(7);
    ...
    if (ep->is_in == 1U)
    {
        ...
        else
        {
            ...
            */
            ...
            USBx_INEP(ep->num)->DIEPTSIZ &= ~(USB_OTG_DIEPTSIZ_PKTCNT);
            USBx_INEP(ep->num)->DIEPTSIZ |= (USB_OTG_DIEPTSIZ_PKTCNT & (((
                                                 ep->xfer_len + ep->maxpacket - 1U) / ep->maxpacket) << 19U)) ;
...
static HAL_StatusTypeDef USB_CoreReset(USB_OTG_GlobalTypeDef *USBx)
{
    ...
    while ((USBx->GRSTCTL & USB_OTG_GRSTCTL_CSRST) != 0);
    while ((USBx->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0);
    return HAL_OK;
}

/**
  * @brief Init the USB phy
  * @param  USBx Selected device
  */
static void USB_PhyInit(USB_OTG_GlobalTypeDef *USBx)
{
    USBx->GUSBCFG |= USB_OTG_GUSBCFG_FHMOD;// | USB_OTG_GUSBCFG_SRPCAP | USB_OTG_GUSBCFG_HNPCAP;
    USBx->GUSBCFG &= ~(USB_OTG_GUSBCFG_TRDT);
    USBx->GUSBCFG |=  USB_OTG_GUSBCFG_TRDT_3 | USB_OTG_GUSBCFG_TRDT_0;
}
                               ...
                               USB_OTG_CfgTypeDef cfg)
{
    ...
    USBx->GINTMSK = (1 << 24 | 1 << 25 | 1 <<
                     29); //USBC_PRTINT | USBC_HCHINT | USBC_DISCONNINT;
#endif
    return HAL_OK;
}
...
HAL_StatusTypeDef USB_ResetPort(USB_OTG_GlobalTypeDef *USBx)
{
    ...
    for (uint32_t i = 0; i < 0xfffff; i++)
        __NOP();
#else
    USB_Delay(1000U);                                 /* See Note #1 */
#endif
    USBx_HPRT0 = ((~USB_OTG_HPRT_PRST) & hprt0);
    return HAL_OK;
}
#if 0
void USB_PortReset(uint32_t ms)
{
    int lock;
...
#else
            USBx_HC(ch_num)->HCINTMSK = USB_OTG_HCINTMSK_AHBERR  | USB_OTG_HCINTMSK_CHHM;
#endif
            break;
        default:
            break;
...
HAL_StatusTypeDef USB_HC_Halt(USB_OTG_GlobalTypeDef *USBx, uint8_t hc_num)
{
    ...
    else
    {
        ...
        {
            if ((((USBx_HC(hc_num)->HCCHAR) & USB_OTG_HCCHAR_EPTYP) >> 18) != HCCHAR_ISOC) {
                if ((USBx_HC(hc_num)->HCCHAR & USB_OTG_HCCHAR_CHENA)) {
                    USBx_HC(hc_num)->HCCHAR |= USB_OTG_HCCHAR_CHDIS;
                }
            }
            //USBx_HC(hc_num)->HCCHAR |= USB_OTG_HCCHAR_CHENA;
...
HAL_StatusTypeDef USB_StopHost(USB_OTG_GlobalTypeDef *USBx)
{
    ...
    for (i = 0; i <= 15; i++)
    {
        ...
        if (((((USBx_HC(i)->HCCHAR) & USB_OTG_HCCHAR_EPTYP) >> 18) == HCCHAR_ISOC)) {
            value = USBx_HC(i)->HCCHAR ;
            if (value & USB_OTG_HCCHAR_CHENA) {
