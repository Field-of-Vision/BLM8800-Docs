...
#ifdef CFG_IPERF_TCP_OPT
#define IPERF_TCP_MAX_IDLE_SEC        20
#else
...
__SHAREDRAM static char net_iperf_nocopy_buf[IPERF_BUFFER_LEN];
/// Pool of IPERF send buffers
__SHAREDRAM static struct net_iperf_send_buf_tag
net_iperf_send_buf_mem[FHOST_IPERF_MAX_STREAMS][FHOST_IPERF_SEND_BUF_CNT];
                                ...
                                struct fhost_iperf_settings* iperf_settings)
{
    ...
    do
    {
        ...
        #ifdef CFG_IPERF_TCP_OPT
        if ((TCP_SND_BUF >= (TCP_MSS * 8)) && (pcb->snd_buf <= (TCP_MSS * 4))) {
            write_more = 0;
        }
        #endif
...
static err_t net_iperf_tcp_server_accept(void * arg, struct tcp_pcb *newpcb, err_t err)
{
    ...
    #ifdef CFG_IPERF_TCP_OPT
    tcp_poll(newpcb, net_iperf_tcp_poll, 1U);
    #else
        ...
        const ip_addr_t *addr, u16_t port)
{
    ...
    else
    {
        ...
        net_if_get_ip(n, &ip, NULL, NULL);
        ip4_addr_t *addr4 = ip_2_ip4((ip_addr_t *)addr);

        dbg("local %d.%d.%d.%d port %d connected with %d.%d.%d.%d port %d\n",
                ip & 0xff, (ip >> 8) & 0xff, (ip >> 16) & 0xff, (ip >> 24) & 0xff,
                stream->iperf_settings.port, addr4->addr & 0xff, (addr4->addr >> 8) & 0xff,
                (addr4->addr >> 16) & 0xff, (addr4->addr >> 24) & 0xff, port);
        aic_time_get(SINCE_EPOCH, &report->start_time.sec, &report->start_time.usec);
        if (iperf_settings->interval.sec != 0 || iperf_settings->interval.usec != 0)
        {
            iperf_timeradd(&report->start_time, &iperf_settings->interval, &report->interval_target);
...
static err_t net_iperf_pcb_config(void *pcb, struct fhost_iperf_stream *stream)
{
    ...
    else
    {
        ...
        else
        {
            ...
            #ifdef CFG_IPERF_TCP_OPT
            tcp_poll((struct tcp_pcb *) pcb, net_iperf_tcp_poll, 1U);
            #else
...
static void* net_iperf_init(struct fhost_iperf_stream* stream)
{
    ...
    if (!stream->iperf_settings.flags.is_server)
    {
        ...
        ip4_addr_set_u32(&rip,  ip);

        /* Don't call ip_route() with IP_ANY_TYPE */
        netif = ip4_route_src(ip_2_ip4(IP46_ADDR_ANY(IPADDR_TYPE_V4)), &rip);
