...
#define DAC_SRC_TX_DMA_CH    DMA_CHANNEL_AUD_STREAM0_PLAYBACK
#define DAC_SRC_RX_DMA_CH    DMA_CHANNEL_AUD_STREAM0_CAPTURE
#define ADC_SRC_TX_DMA_CH    DMA_CHANNEL_AUD_STREAM1_PLAYBACK
#define ADC_SRC_RX_DMA_CH    DMA_CHANNEL_AUD_STREAM1_CAPTURE
...
static void aud_dac_src_tx_dma_irq_handler(void);
static void aud_dac_src_rx_dma_irq_handler(void);
static void aud_adc_src_tx_dma_irq_handler(void);
static void aud_adc_src_rx_dma_irq_handler(void);
...
int aud_adc_src_out_initial(unsigned int aud_adc_src_rx_addr_sta)
{
    ...
    dma_erqcsr_set(REQ_CID_AUD_SRC_TX,ADC_SRC_TX_DMA_CH);
    dma_ch_rqr_erqm_setb(ADC_SRC_TX_DMA_CH);
    dma_ch_rqr_erql_setb(ADC_SRC_TX_DMA_CH);
    dma_ch_icsr_tbl2_iena_setb(ADC_SRC_TX_DMA_CH);
    dma_ch_icsr_tll_iena_setb(ADC_SRC_TX_DMA_CH);
    dma_ch_lnar_set(ADC_SRC_TX_DMA_CH,(unsigned int)&dma_desc0);
    dma_ch_ctlr_set(ADC_SRC_TX_DMA_CH, DMA_CH_CHENA_BIT | DMA_CH_LSTENA_BIT | (0x01UL << DMA_CH_BUSBU_LSB));
    ///aud src txrx_channel
    dma_ch_sar_set(ADC_SRC_RX_DMA_CH,(REG_AUD_PROC_BASE | (0x6<<8 | 0x2<<4)));
    dma_ch_dar_set(ADC_SRC_RX_DMA_CH,aud_adc_src_rx_addr_sta);
    dma_ch_tbl0cr_set(ADC_SRC_RX_DMA_CH, ((16) | (REQ_TBL0 << DMA_CH_RQTYP_LSB) | (AHB_WORD << DMA_CH_DBUSU_LSB) |
                             (AHB_WORD << DMA_CH_SBUSU_LSB) | DMA_CH_CONSTSA_BIT));
    dma_ch_tbl1cr_set(ADC_SRC_RX_DMA_CH,(((aud_src_rx_len) << DMA_CH_TBL1_CNT_LSB) & DMA_CH_TBL1_CNT_MASK));
    dma_ch_tbl2cr_set(ADC_SRC_RX_DMA_CH,(((aud_src_rx_len<<2) << DMA_CH_TBL2_CNT_LSB) & DMA_CH_TBL2_CNT_MASK));
    dma_ch_tsr_set(ADC_SRC_RX_DMA_CH, ((4 << DMA_CH_STRANSZ_LSB) | (4 << DMA_CH_DTRANSZ_LSB)));
    dma_ch_wmar_set(ADC_SRC_RX_DMA_CH, 0);
    dma_ch_wjar_set(ADC_SRC_RX_DMA_CH, 0);
    dma_ch_tbl0sr_set(ADC_SRC_RX_DMA_CH, ((0 << DMA_CH_STBL0SZ_LSB) | (0 << DMA_CH_DTBL0SZ_LSB)));
    dma_ch_tbl1ssr_set(ADC_SRC_RX_DMA_CH, 0);
    dma_ch_tbl1dsr_set(ADC_SRC_RX_DMA_CH, 0);
    dma_erqcsr_set(REQ_CID_AUD_SRC_RX, ADC_SRC_RX_DMA_CH);
    dma_ch_rqr_erqm_setb(ADC_SRC_RX_DMA_CH);
    dma_ch_rqr_erql_setb(ADC_SRC_RX_DMA_CH);
    dma_ch_icsr_tbl2_iena_setb(ADC_SRC_RX_DMA_CH);

    NVIC_SetVector(DMA00_IRQn + ADC_SRC_TX_DMA_CH, (uint32_t)aud_adc_src_tx_dma_irq_handler);
    NVIC_SetVector(DMA00_IRQn + ADC_SRC_RX_DMA_CH, (uint32_t)aud_adc_src_rx_dma_irq_handler);
    NVIC_SetPriority(DMA00_IRQn + ADC_SRC_TX_DMA_CH, __NVIC_PRIO_LOWEST);
    NVIC_SetPriority(DMA00_IRQn + ADC_SRC_RX_DMA_CH, __NVIC_PRIO_LOWEST);
    NVIC_EnableIRQ(DMA00_IRQn + ADC_SRC_TX_DMA_CH);
    NVIC_EnableIRQ(DMA00_IRQn + ADC_SRC_RX_DMA_CH);
#elif PLF_AIC8800MC
    //TODO
#elif PLF_AIC8800M40
    //TODO
...
int aud_dac_src_out_initial(unsigned int aud_dac_src_rx_addr_sta)
{
    ...
    dma_erqcsr_set(REQ_CID_AUD_PROC_TX0,DAC_SRC_TX_DMA_CH);
    dma_ch_rqr_erqm_setb(DAC_SRC_TX_DMA_CH);
    dma_ch_rqr_erql_setb(DAC_SRC_TX_DMA_CH);
    dma_ch_icsr_tbl2_iena_setb(DAC_SRC_TX_DMA_CH);
    dma_ch_icsr_tll_iena_setb(DAC_SRC_TX_DMA_CH);
    dma_ch_lnar_set(DAC_SRC_TX_DMA_CH,(unsigned int)&dma_desc2);
    dma_ch_ctlr_set(DAC_SRC_TX_DMA_CH, DMA_CH_CHENA_BIT | DMA_CH_LSTENA_BIT | (0x01UL << DMA_CH_BUSBU_LSB));
    ///aud src txrx_channel
    dma_ch_sar_set(DAC_SRC_RX_DMA_CH,(REG_AUD_PROC_BASE | (0x5<<8 | 0x2<<4)));
    dma_ch_dar_set(DAC_SRC_RX_DMA_CH,aud_dac_src_rx_addr_sta);
    dma_ch_tbl0cr_set(DAC_SRC_RX_DMA_CH, ((16) | (REQ_TBL0 << DMA_CH_RQTYP_LSB) | (AHB_WORD << DMA_CH_DBUSU_LSB) |
                             (AHB_WORD << DMA_CH_SBUSU_LSB) | DMA_CH_CONSTSA_BIT));
    dma_ch_tbl1cr_set(DAC_SRC_RX_DMA_CH,(((aud_src_rx_len) << DMA_CH_TBL1_CNT_LSB) & DMA_CH_TBL1_CNT_MASK));
    dma_ch_tbl2cr_set(DAC_SRC_RX_DMA_CH,(((aud_src_rx_len<<2) << DMA_CH_TBL2_CNT_LSB) & DMA_CH_TBL2_CNT_MASK));
    dma_ch_tsr_set(DAC_SRC_RX_DMA_CH, ((4 << DMA_CH_STRANSZ_LSB) | (4 << DMA_CH_DTRANSZ_LSB)));
    dma_ch_wmar_set(DAC_SRC_RX_DMA_CH, 0);
    dma_ch_wjar_set(DAC_SRC_RX_DMA_CH, 0);
    dma_ch_tbl0sr_set(DAC_SRC_RX_DMA_CH, ((0 << DMA_CH_STBL0SZ_LSB) | (0 << DMA_CH_DTBL0SZ_LSB)));
    dma_ch_tbl1ssr_set(DAC_SRC_RX_DMA_CH, 0);
    dma_ch_tbl1dsr_set(DAC_SRC_RX_DMA_CH, 0);
    dma_erqcsr_set(REQ_CID_AUD_PROC_TXRX0, DAC_SRC_RX_DMA_CH);
    dma_ch_rqr_erqm_setb(DAC_SRC_RX_DMA_CH);
    dma_ch_rqr_erql_setb(DAC_SRC_RX_DMA_CH);
    dma_ch_icsr_tbl2_iena_setb(DAC_SRC_RX_DMA_CH);

    NVIC_SetVector(DMA00_IRQn + DAC_SRC_TX_DMA_CH, (uint32_t)aud_dac_src_tx_dma_irq_handler);
    NVIC_SetVector(DMA00_IRQn + DAC_SRC_RX_DMA_CH, (uint32_t)aud_dac_src_rx_dma_irq_handler);
    NVIC_SetPriority(DMA00_IRQn + DAC_SRC_TX_DMA_CH, __NVIC_PRIO_LOWEST);
    NVIC_SetPriority(DMA00_IRQn + DAC_SRC_RX_DMA_CH, __NVIC_PRIO_LOWEST);
    NVIC_EnableIRQ(DMA00_IRQn + DAC_SRC_TX_DMA_CH);
    NVIC_EnableIRQ(DMA00_IRQn + DAC_SRC_RX_DMA_CH);
#elif PLF_AIC8800MC
    //TODO
#elif PLF_AIC8800M40
    //TODO
...
void adc_src_rx_dma_start(unsigned int aud_adc_src_rx_addr_sta)
{
    ...
    dma_ch_tbl2cr_set(ADC_SRC_RX_DMA_CH,(((aud_src_rx_len<<2) << DMA_CH_TBL2_CNT_LSB) & DMA_CH_TBL2_CNT_MASK));
    dma_erqcsr_set(REQ_CID_AUD_SRC_RX, ADC_SRC_RX_DMA_CH);
    dma_ch_ctlr_set(ADC_SRC_RX_DMA_CH, (DMA_CH_CHENA_BIT | (0x01UL << DMA_CH_BUSBU_LSB)));
#elif PLF_AIC8800MC
    //TODO
#elif PLF_AIC8800M40
    //TODO
...
void dac_src_rx_dma_start(unsigned int aud_dac_src_rx_addr_sta)
{
    ...
    dma_ch_tbl2cr_set(DAC_SRC_RX_DMA_CH,(((aud_src_rx_len<<2) << DMA_CH_TBL2_CNT_LSB) & DMA_CH_TBL2_CNT_MASK));
    dma_erqcsr_set(REQ_CID_AUD_PROC_TXRX0, DAC_SRC_RX_DMA_CH);
    dma_ch_ctlr_set(DAC_SRC_RX_DMA_CH, (DMA_CH_CHENA_BIT | (0x01UL << DMA_CH_BUSBU_LSB)));
#elif PLF_AIC8800MC
    //TODO
#elif PLF_AIC8800M40
    //TODO
...
static void aud_dac_src_tx_dma_irq_handler(void)
{
    dma_cx_common_irq_handler(DAC_SRC_TX_DMA_CH);
}

static void aud_dac_src_rx_dma_irq_handler(void)
{
    dma_cx_common_irq_handler(DAC_SRC_RX_DMA_CH);
}

static void aud_adc_src_tx_dma_irq_handler(void)
{
    dma_cx_common_irq_handler(ADC_SRC_TX_DMA_CH);
}

static void aud_adc_src_rx_dma_irq_handler(void)
{
    dma_cx_common_irq_handler(ADC_SRC_RX_DMA_CH);
}
...
void asio_dsd_init(unsigned int *adc_tx_0,unsigned int *adc_tx_1,unsigned int *dac_tx_0,unsigned int *dac_tx_1)
{
    ...
    dma_cx_handler_register(ADC_SRC_TX_DMA_CH,aud_adc_src_tx_handler);
    dma_cx_handler_register(ADC_SRC_RX_DMA_CH,aud_adc_src_rx_handler);
    dma_cx_handler_register(DAC_SRC_TX_DMA_CH,aud_dac_src_tx_handler);
    dma_cx_handler_register(DAC_SRC_RX_DMA_CH,aud_dac_src_rx_handler);
