...
static void mqtt_ack_handler_resend(mqtt_client_t* c, ack_handlers_t* ack_handler)
{
    ...
    MQTT_LOG_W("%s:%d\n%s()... <resend %d package, packet_id is %d> ", __FILE__, __LINE__, __FUNCTION__, ack_handler->type, ack_handler->packet_id);
...
static int mqtt_msg_handler_is_exist(mqtt_client_t* c, message_handlers_t *handler)
{
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        ...
        if ((NULL != msg_handler->topic_filter) && (mqtt_is_topic_equals(msg_handler->topic_filter, handler->topic_filter))) {
            MQTT_LOG_W("%s:%d\n%s()... <msg_handler->topic_filter: %s, handler->topic_filter: %s>",
...
static int mqtt_try_resubscribe(mqtt_client_t* c)
{
    ...
    MQTT_LOG_W("%s:%d\n%s()... <mqtt try resubscribe ...>", __FILE__, __LINE__, __FUNCTION__);
    ...
    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        ...
        if ((rc = mqtt_subscribe(c, msg_handler->topic_filter, msg_handler->qos, msg_handler->handler)) == MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR)
            MQTT_LOG_W("%s:%d\n%s()... <mqtt ack handler num too much ...>", __FILE__, __LINE__, __FUNCTION__);
...
static int mqtt_try_do_reconnect(mqtt_client_t* c)
{
    ...
    MQTT_LOG_D("%s:%d\n%s()... <mqtt try connect result is -0x%04x>", __FILE__, __LINE__, __FUNCTION__, -rc);
...
static int mqtt_yield(mqtt_client_t* c, int timeout_ms)
{
    ...
    while (!platform_timer_is_expired(&timer)) {
        ...
        if (rc >= 0) {
            ...
            MQTT_LOG_E("%s:%d\n%s()... <mqtt not connect>", __FILE__, __LINE__, __FUNCTION__);
        } else {
            break;
        }
    }
...
static void mqtt_yield_thread(void *arg)
{
    ...
    if (CLIENT_STATE_CONNECTED !=  state) {
        MQTT_LOG_W("%s:%d\n%s()... <mqtt is not connected to the server...>\r\n",  __FILE__, __LINE__, __FUNCTION__);
...
static int mqtt_connect_with_results(mqtt_client_t* c)
{
    ...
    #ifndef MQTT_NETWORK_TYPE_TLS_TWO_WAY
    rc = network_init(c->mqtt_network, c->mqtt_host, c->mqtt_port, c->mqtt_ca, NULL, NULL);
    #else
    rc = network_init(c->mqtt_network, c->mqtt_host, c->mqtt_port, c->mqtt_ca, c->mqtt_client_cert, c->mqtt_client_key);
    #endif
#else
    ...
    dbg("%s:%d\n%s()... <mqtt connect success...>", __FILE__, __LINE__, __FUNCTION__);
...
exit:
    ...
    if (rc == MQTT_SUCCESS_ERROR) {
        if(NULL == c->mqtt_thread) {
            ...
            c->mqtt_thread= platform_thread_init("mqtt_yield_thread", mqtt_yield_thread, c, TASK_STACK_SIZE_MQTT, TASK_PRIORITY_MQTT, MQTT_THREAD_TICK);
...
static uint32_t mqtt_write_buf_malloc(mqtt_client_t* c, uint32_t size)
{
    ...
    if (NULL == c->mqtt_write_buf) {
        MQTT_LOG_E("%s:%d\n%s()... <malloc write buf failed...>", __FILE__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
    }
...
static int mqtt_init(mqtt_client_t* c)
{
    ...
    if (NULL == c->mqtt_network) {
        MQTT_LOG_E("%s:%d\n%s()... <malloc memory failed...>", __FILE__, __LINE__, __FUNCTION__);
        RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
    }
...
MQTT_CLIENT_SET_DEFINE(client_cert, char*, NULL)
MQTT_CLIENT_SET_DEFINE(client_key, char*, NULL)
...
int mqtt_keep_alive(mqtt_client_t* c)
{
    ...
    if (platform_timer_is_expired(&c->mqtt_last_sent) || platform_timer_is_expired(&c->mqtt_last_received)) {
        if (c->mqtt_ping_outstanding) {
            MQTT_LOG_W("%s:%d\n%s()... <ping outstanding>", __FILE__, __LINE__, __FUNCTION__);
            /*must realse the socket file descriptor zhaoshimin 20200629*/
            #ifdef MQTT_NETWORK_TYPE_NO_TLS
            network_release(c->mqtt_network);
            #else
            nettype_tls_disconnect(c->mqtt_network);
            #endif
            ...
            if (len > 0 && (rc = mqtt_send_packet(c, len, &timer)) == MQTT_SEND_PACKET_ERROR) // send the ping packet
...
int mqtt_release(mqtt_client_t* c)
{
    ...
    while ((CLIENT_STATE_INVALID != mqtt_get_client_state(c)) && \
            (CLIENT_STATE_INITIALIZED != mqtt_get_client_state(c))) {
        // platform_timer_usleep(1000);            // 1ms avoid compiler optimization.
        if (platform_timer_is_expired(&timer)) {
            MQTT_LOG_E("%s:%d\n%s()... <mqtt release failed...>", __FILE__, __LINE__, __FUNCTION__);
            RETURN_ERROR(MQTT_FAILED_ERROR)
        }
    }
...
exit:
    ...
    if ((MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR == rc) || (MQTT_MEM_NOT_ENOUGH_ERROR == rc)) {
        MQTT_LOG_W("%s:%d\n%s()... <there is not enough memory space to record...>", __FILE__, __LINE__, __FUNCTION__);
...
int mqtt_list_subscribe_topic(mqtt_client_t* c)
{
    ...
    if (mqtt_list_is_empty(&c->mqtt_msg_handler_list))
        MQTT_LOG_I("%s:%d\n%s()... <there are no subscribed topics...>", __FILE__, __LINE__, __FUNCTION__);

    LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
        msg_handler = LIST_ENTRY(curr, message_handlers_t, list);
        /* determine whether a node already exists by mqtt topic, but wildcards are not supported */
        if (NULL != msg_handler->topic_filter) {
            MQTT_LOG_I("%s:%d\n%s()... <[%d] subscribe topic: %s>", __FILE__, __LINE__, __FUNCTION__, ++i ,msg_handler->topic_filter);
        }
    }
