...
static rtos_mutex co_timer_mutex;
...
{
    int ret = rtos_timer_change_period(co_main_timer, t, 1);
    if(ret)
        dbg("co main timer error\n");
}
/**
 ****************************************************************************************
 * @brief * basic timer function for  co_timer .Users don't have to pay attention to it
...
static void co_main_timer_init(void)
{
    ...
    if (rtos_mutex_recursive_create(&co_timer_mutex))
    {
        ASSERT_ERR(0);
    }
}
...
static void co_timer_list_reset(void)
{
    ...
    rtos_mutex_recursive_lock(co_timer_mutex);
    ...
    rtos_mutex_recursive_unlock(co_timer_mutex);
    COTIMER_DEBUG_PRINTF("duration %d ,now 0x%x,list_len %x\n",min_duration,time_now,list_len);
    ...
    if(list_len){
        ...
        while(element != NULL){
            ...
            if(elapsed_time >= current_timer->time_duration){
                ...
                COTIMER_DEBUG_PRINTF("%s tmp_timer free 0x%x ,cb 0x%x,period %x\n",__func__,current_timer,cb,current_timer->is_period);
