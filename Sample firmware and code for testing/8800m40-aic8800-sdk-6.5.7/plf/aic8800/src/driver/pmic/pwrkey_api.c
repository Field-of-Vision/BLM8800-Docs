...
void pwrkey_init(void)
{
    if (user_pwrkey_enabled()) {
        ...
        if (user_pwrkey_poweron_time_get() && (key_held == 0)) {
            uint32_t reg = pmic_por_source_get();
            PWRKEY_DEBUG_PRINTF("Start Reason: %x\n", reg);
            //uart_wait_tx_done();
            if (reg != 0x40)  // AIC8800M: reason code, when use pmic_chip_reboot.
                pmic_chip_shutdown(); // pwrkey release
        }
