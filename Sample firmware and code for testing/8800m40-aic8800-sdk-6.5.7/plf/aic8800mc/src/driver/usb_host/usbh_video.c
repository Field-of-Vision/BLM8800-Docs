...
VIDEO_HandleTypeDef USBH_Video_Handle;
...
static USBH_StatusTypeDef USBH_VIDEO_InterfaceInit (USBH_HandleTypeDef *phost)
{
  ...
  else
  {
    ...
    #else
    phost->pActiveClass->pData = (VIDEO_HandleTypeDef *)&USBH_Video_Handle;
    #endif
...
#if (USBH_USE_OS == 1)
      rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_URB_EVENT), false);
#endif
    }
    break;
