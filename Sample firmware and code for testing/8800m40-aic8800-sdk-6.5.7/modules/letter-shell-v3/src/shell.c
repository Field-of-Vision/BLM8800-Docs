...
void shellPrint(Shell *shell, char *fmt, ...)
{
    ...
    va_start(vargs, fmt);
    dbg_vsnprintf(buffer, SHELL_PRINT_BUFFER - 1, fmt, vargs);
    va_end(vargs);
...
void shellEnter(Shell *shell)
{
    ...
    if (shell->parser.length == 0) {
        ...
        // indicate candidate cmd
        SHELL_ASSERT(shell->indicate, return);
        shell->indicate();
    }
#endif
}
...
void shellTask(void *param)
{
    ...
    SHELL_ASSERT(shell->wait, return);
#endif
...
#else
        // wait for semaphore and execute
        shell->wait();
        shellExec(shell);
        shellWriteCommandLine(shell, 1);
#endif
