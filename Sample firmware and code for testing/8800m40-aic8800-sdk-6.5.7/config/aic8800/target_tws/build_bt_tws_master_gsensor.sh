...
python ../../../tools/scons.py -u . PRODUCT=host-bt PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" HW_PLATFORM=asic \
        TGT_MODE=tws \
        AON=on CONSOLE=on TGTNAME=tws_master_g BT_PATCH_MODE=tws\
        BT_AUDIO=on AUD_USED=on APPS=on BT_TWS=on TWS_ROLE=master BT_OTA=on \
        KEY_MODULE=on LED_MODULE=off GSENSOR=on LIGHT_SENSOR=on I2CM_SFT=on\
