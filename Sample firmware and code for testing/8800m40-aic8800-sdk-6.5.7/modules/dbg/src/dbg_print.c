...
int uart_printf(const char *format, va_list arg)
{
    ...
    int size;
    #if (PLF_STDLIB)
    size = vsnprintf(buffer, sizeof(buffer), format, arg);
    #else
    size = dbg_vsnprintf(buffer, sizeof(buffer), format, arg);
    #endif
    if (size > 0) {
...
void dump_mem(uint32_t addr, uint32_t count, uint8_t width, bool ascii_on)
{
    ...
    do {
        ...
        if (IS_PMIC_MEM_ADDR(addr)) {
            ...
            for (i = 0; i < linebytes; i += 4) {
                ...
                if(width == 4) {
                    ...
                    dbg(" %04X %04X", (uint16_t)rdata, (uint16_t)(rdata >> 16));
                } else {
                    dbg(" %02X %02X %02X %02X", (uint8_t)rdata, (uint8_t)(rdata >> 8),
        ...
        for (i = 0; i < linebytes; i += width) {
            if(width == 4) {
                dbg(" %08X", *((uint32_t *)addr));
            } else if(width == 2) {
                dbg(" %04X", *((uint16_t *)addr));
            } else {
                dbg(" %02X", *((uint8_t *)addr));
            }
            addr += width;
        }
...
uint32_t dbg_vsnprintf(char *buffer, uint32_t size, const char *fmt, va_list args)
{
    ...
    #else
    return dbg_vsnprintf_offset(buffer, size, 0, fmt, args);
    #endif
}
...
uint32_t dbg_snprintf(char *buffer, uint32_t size, const char *fmt, ...)
{
    ...
    #if (PLF_STDLIB)
    ret = vsnprintf(buffer, size, fmt_usr, args);
    #else
...
/**
 ****************************************************************************************
 * @brief Execute a pseudo vsprintf function
 *
 * @param[out] buffer  Output buffer
 * @param[in]  fmt     Format string
 * @param[in]  args    Variable list of arguments
...
uint32_t dbg_vsprintf(char *buffer, const char *fmt, va_list args)
{
    ...
    #define WRITE_CHAR(_c) *buffer++ = (_c); res++

    // Check parameters: If buffer is NULL return directly.
    if (buffer == NULL)
        return res;
...
/**
 ****************************************************************************************
 * @brief Execute a pseudo sprintf function
 *
 * @param[out] buffer  Output buffer
 * @param[in]  fmt     Format string
 *
...
uint32_t dbg_sprintf(char *buffer, const char *fmt, ...)
{
    ...
    ret = dbg_vsprintf(buffer, fmt_usr, args);
