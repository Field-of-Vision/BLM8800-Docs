...
#endif
    batt_vl = volt_mv / 1000.0;
    // Calculate Current Battery Level
    float data = (batt_vl - batt_min) / (batt_max - batt_min) * 100.0;
    batt_level = (uint32_t) data;
    batt_level = batt_level / 5 * 5;
    dbg("Current VBAT voltage is %dmV, batt_level is %d\n", volt_mv, batt_level);
    app_batt_env.batt_lvl = batt_level;
    return batt_level;
}
...
void app_batt_send_lvl(uint8_t batt_lvl)
{
    ...
    struct bass_batt_level_upd_req * req = KE_MSG_ALLOC(BASS_BATT_LEVEL_UPD_REQ,
                                                        KE_BUILD_ID(prf_get_task_from_id(TASK_ID_BASS),app_batt_env.conidx),
                                                        KE_BUILD_ID(TASK_APP,app_batt_env.conidx),
