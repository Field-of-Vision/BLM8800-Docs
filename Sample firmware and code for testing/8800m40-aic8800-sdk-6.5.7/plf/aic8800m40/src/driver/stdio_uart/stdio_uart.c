...
#if (PLF_HW_ASIC == 1) || (PLF_HW_FPGA == 1)
...
void stdio_uart_init(void)
{
    ...
    iomux_gpioa_config_sel_setf(4, 0x03);
    iomux_gpioa_config_sel_setf(5, 0x03);
    #elif UART_INDEX == 2
    /* IOMUX UART2_RX = GPIO6    UART2_TX = GPIO7 */
    iomux_gpioa_config_sel_setf(6, 0x02);
    iomux_gpioa_config_sel_setf(7, 0x02);
    #endif
...
__RAMTEXT void stdio_uart_recover(void)
{
    if (!stdio_uart_rxirqen_getf()) {
        stdio_uart_dbufth_pack(0,  //tx data buffer trigger threshold
                               1); //rx data buffer trigger threshold
        stdio_uart_clk_p_setf(1);  //uart clk 0 = 24M, 1 = 48M
        stdio_uart_format_set(8, 0, 1);
        // baud rate
        {
            ...
            div  = (clk + (stdio_uart_baudrate >> 1)) / stdio_uart_baudrate;
            div0 = 0xFFUL & (div >> 4);
            div1 = 0xFFUL & (div >> 12);
