...
void gpioa_force_pull_up_enable(int gpidx)
{
    if (gpidx < 16) {
        ...
        iomux_agpio_config_pull_dn_setf(gpidx - 16, 0);
        iomux_agpio_config_pull_up_setf(gpidx - 16, 1);
        iomux_agpio_config_pull_frc_setf(gpidx - 16, 1);
    }
}
...
void gpioa_force_pull_dn_enable(int gpidx)
{
    if (gpidx < 16) {
        ...
        iomux_agpio_config_pull_up_setf(gpidx - 16, 0);
        iomux_agpio_config_pull_dn_setf(gpidx - 16, 1);
        iomux_agpio_config_pull_frc_setf(gpidx - 16, 1);
    }
}
...
void gpioa_force_pull_up_dn_disable(int gpidx)
{
    if (gpidx < 16) {
        ...
        iomux_agpio_config_pull_up_setf(gpidx - 16, 0);
        iomux_agpio_config_pull_dn_setf(gpidx - 16, 0);
        iomux_agpio_config_pull_frc_setf(gpidx - 16, 0);
    }
}
...
void gpioa_irq_init(int gpidx, int type, gpio_irq_handler_t handler)
{
    ...
    AIC_GPIO->IRR  =  (0x01UL << gpidx); // int clr
    AIC_GPIO->ICR |=  (0x01UL << gpidx); // int en
    ...
    if (!NVIC_GetEnableIRQ(GPIO_IRQn)) {
        NVIC_SetVector(GPIO_IRQn, (uint32_t)gpioa_isr);
...
static HWP_AIC1000LITE_GPIO_T * const PMIC_GPIOB = aic1000liteGpio;
#endif
#if PLF_PMIC_VER_AUD
...
void gpiob_init(int gpidx)
{
    ...
    // iomux
    if (gpidx < 2) {
        iomux_gpiob_config_sel_setf(gpidx, AIC1000LITE_IOMUX_PAD_GPIO_0_SEL_FUN_GPIO_0_SEL);
    } else if (gpidx < 16) {
        iomux_gpiob_config_sel_setf(gpidx, AIC1000LITE_IOMUX_PAD_GPIO_2_SEL_FUN_GPIO_2_SEL);
    ...
    // iomux
    if (gpidx < 2) {
        iomux_gpiob_config_sel_setf(gpidx, AIC1000AUD_IOMUX_PAD_GPIO_0_SEL_FUN_GPIO_0_SEL);
    } else if (gpidx < 8) {
        iomux_gpiob_config_sel_setf(gpidx, AIC1000AUD_IOMUX_PAD_GPIO_2_SEL_FUN_GPIO_2_SEL);
    } else {
        return;
    }
#endif
...
void gpiob_irq_init(int gpidx, int type, gpio_irq_handler_t handler, int filter)
{
    ...
    gpiob_init(gpidx);
    gpiob_dir_in(gpidx);
    // If used gpiob2/3, need to pull up first, and then outside IO pull it down.
    #if PLF_PMIC_VER_LITE
    if (gpidx == 2 || gpidx == 3)
        gpiob_force_pull_up_enable(gpidx);
    #endif
    ...
    if (pwrctrl_pwrmd_cpusys_sw_record_getf() >= CPU_SYS_POWER_DOWN) {
        PMIC_MEM_WRITE((unsigned int)(&PMIC_GPIOB->IRR), gpmsk); // int clr
    }
...
void gpiob_irq_deinit(int gpidx)
{
    ...
    PMIC_MEM_MASK_WRITE((unsigned int)(&PMIC_GPIOB->ICR), 0, gpmsk); // int dis
    gpiob_deinit(gpidx);
}
