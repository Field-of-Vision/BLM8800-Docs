...
int fhost_application_init(void)
{
    ...
    wifi_engineering_mode_status_set(wifi_mic_config_enable_em_chan_get());
    wifi_engineering_mode_channel_index_set(wifi_mic_config_em_chan_index_get());
    #endif

    #if WIFI_VOICE_ROLE_DONGLE
    if (0 != wifi_mic_rx_init()) {
        return -1;
    }
    if (0 != wifi_mic_rx_user_init()) {
        return -1;
    }
    #else
    if (0 != wifi_mic_tx_init()) {
        return -1;
    }
    if (0 != wifi_mic_tx_user_init()) {
        return -1;
    }
    #endif
...
void fhost_application_deinit(void)
{
    #if WIFI_VOICE_ROLE_DONGLE
    wifi_mic_rx_deinit();
    wifi_mic_rx_user_deinit();
    #else
    wifi_mic_tx_deinit();
    wifi_mic_tx_user_deinit();
    #endif
