...
netdev_tx_t rwnx_start_monitor_if_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    u8_l tid, rate_fmt = FORMATMOD_NON_HT, rate_idx = 0, txsig_bw = PHY_CHNL_BW_20, tx_noack = 0, cts2self = 0;
    u8_l *pframe, *data;
    bool robust;
...
//    printk("rwnx_start_monitor_if_xmit, skb_len=%d, rtap_len=%d\n", skb->len, rtap_len);
    ...
    sw_txhdr->rate_config = ((cts2self == 1) ? PROT_SELF_CTS : PROT_NO_PROT) |
                            (((u32_l)tx_noack << TX_FLAG_NOACK_OFT) & TX_FLAG_NOACK_MASK) |
                            (((u32_l)rate_fmt << FORMAT_MOD_PROT_TX_RCX_OFT) & FORMAT_MOD_PROT_TX_RCX_MASK) |
                            (((u32_l)txsig_bw << BW_PROT_TX_RCX_OFT) & BW_PROT_TX_RCX_MASK) |
                            (((u32_l)rate_idx << MCS_INDEX_PROT_TX_RCX_OFT) & MCS_INDEX_PROT_TX_RCX_MASK) |
                            (((u32_l)rate_fmt << FORMAT_MOD_TX_RCX_OFT) & FORMAT_MOD_TX_RCX_MASK) |
                            (((u32_l)txsig_bw << BW_TX_RCX_OFT) & BW_TX_RCX_MASK) |
                            (((u32_l)rate_idx << MCS_INDEX_TX_RCX_OFT) & MCS_INDEX_TX_RCX_MASK); // from radiotap
    /* Fill the Descriptor to be provided to the MAC SW */
    desc = &sw_txhdr->desc;
    desc->host.staid = (sta) ? sta->sta_idx : 0xFF;
    desc->host.vif_idx = vif->vif_index;
