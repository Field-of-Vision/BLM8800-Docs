...
do
  if [ $var = "-libuse" ]; then
    opt=$opt" USE_LIB_DRV=on USE_LIB_WIFI=on USE_LIB_WPA=on USE_LIB_WAPI=on"
  elif [ $var = "-libgen" ]; then
    opt=$opt" GEN_LIB_DRV=on GEN_LIB_WIFI=on GEN_LIB_WPA=on GEN_LIB_WAPI=on"
  else
    opt=$opt" "$var
  fi
done
...
python ../../../tools/scons.py -u . PRODUCT=host-wifi PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" \
        TGT_MODE=wifi_wapi BAND5G=on CRYPTO=mbedtls WAPI=on HEAP_SIZE=0x14000 \
        SOFTAP=on PMIC_VER=lite FHOST_APP=console TGTNAME=host_wb_wifi_wapi $opt
