...
__STATIC_INLINE void flash_cmd_wait_done(void)
{
    ...
    do {
        val = *((volatile unsigned int *)0x4003800C);
...
int flash_calib_info_read(unsigned int flag, void *buf, unsigned int len)
{
    ...
    if (calib_info.magic_num == FLASH_CALIB_INFO_MAGIC_NUM) {
        ...
        if (info_flag_bits & CALIB_INFO_WIFI2G4_RXGAIN_FLAG_BIT) {
            memcpy((char *)buf + index, &calib_info.wifi2g4_rxgain, sizeof(calib_info.wifi2g4_rxgain));
            index += sizeof(calib_info.wifi2g4_rxgain);
        }
...
{
    return flash_calib_info_read(CALIB_INFO_WIFI2G4_RXGAIN_FLAG_BIT, (void *)rxgain, sizeof(wifi2g4_rxgain_info_t));
}
