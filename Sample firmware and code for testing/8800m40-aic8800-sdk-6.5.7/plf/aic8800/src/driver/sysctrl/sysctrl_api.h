/**
 ****************************************************************************************
 *
 * @file sysctrl_api.h
 *
 * @brief SYSCTRL utility functions
 *
 ****************************************************************************************
 */

#ifndef _SYSCTRL_API_H_
#define _SYSCTRL_API_H_

/*
 * INCLUDE FILES
 ****************************************************************************************
 */
#include "reg_sysctrl.h"
#include "hw_cfg.h"

#define DEF_CLK_FREQUENCY_52M       ( 52000000UL)
#define DEF_CLK_FREQUENCY_26M       ( 26000000UL)
#define SYS_CLK_FREQUENCY_40M       ( 40000000UL)

enum {
    SYS_FCLK = 0,
    SYS_HCLK,
    SYS_PCLK,
    PER_UART0,
    PER_UART1,
    PER_UART2,
    PER_PSRAM,
    PER_FLASH,
    PER_PWM,
};

/* For fclk_div/psram_clk_div only */
enum {
    CLK_DIV_VALUE_24  = 0x0,
    CLK_DIV_VALUE_16  = 0x1,
    CLK_DIV_VALUE_12  = 0x2,
    CLK_DIV_VALUE_8   = 0x3,
    CLK_DIV_VALUE_7   = 0x4,
    CLK_DIV_VALUE_6   = 0x5,
    CLK_DIV_VALUE_5P5 = 0x6,
    CLK_DIV_VALUE_5   = 0x7,
    CLK_DIV_VALUE_4P5 = 0x8,
    CLK_DIV_VALUE_4   = 0x9,
    CLK_DIV_VALUE_3P5 = 0xA,
    CLK_DIV_VALUE_3   = 0xB,
    CLK_DIV_VALUE_2P5 = 0xC,
    CLK_DIV_VALUE_2   = 0xD,
    CLK_DIV_VALUE_24D = 0xE,
    CLK_DIV_VALUE_1   = 0xF,
    CLK_DIV_VALUE_MAX
};

/* Clock divider targets */
enum {
    CLK_DIV_TARGET_FCLK = 0,
    CLK_DIV_TARGET_PSRAM,
    CLK_DIV_TARGET_HCLK,
    CLK_DIV_TARGET_PCLK,
    CLK_DIV_TARGET_MAX
};

/* Clock selection targets */
enum {
    CLK_SEL_TARGET_FCLK_FAST = 0,
    CLK_SEL_TARGET_FCLK_PLL320M,
    CLK_SEL_TARGET_FCLK_PLLFLEX,
    CLK_SEL_TARGET_UART0_PCLK,
    CLK_SEL_TARGET_UART1_PCLK,
    CLK_SEL_TARGET_UART2_PCLK,
    CLK_SEL_TARGET_FLASH_FCLK,
    CLK_SEL_TARGET_PSRAM_PLLFLEX,
    CLK_SEL_TARGET_MAX
};

enum {                              // Note: F120 (1~4) could not be supported, please consult the vendor
    CLK_CFG_D480S240P120F120 = 0,   // F120 (1)
    CLK_CFG_D480S240P120F80,
    CLK_CFG_D480S240P120F60,
    CLK_CFG_D480S240P120F40,
    CLK_CFG_D480S240P60F120,        // F120 (2)
    CLK_CFG_D480S240P60F80,
    CLK_CFG_D480S240P60F60,
    CLK_CFG_D480S240P60F40,
    CLK_CFG_D240S240P120F120,       // F120 (3)
    CLK_CFG_D240S240P120F60,        // default
    CLK_CFG_D240S240P120F40,
    CLK_CFG_D240S240P60F60,
    CLK_CFG_D240S240P60F40,
    CLK_CFG_D240S120P60F120,        // F120 (4)
    CLK_CFG_D240S120P60F60,
    CLK_CFG_D240S120P60F40,
    CLK_CFG_D240S120P30F60,
    CLK_CFG_D240S120P30F40,
    CLK_CFG_D192S192P96F96,
    CLK_CFG_D192S192P96F48,
    CLK_CFG_D192S192P48F96,
    CLK_CFG_D192S192P48F48,
    CLK_CFG_D120S120P60F60,
    CLK_CFG_D120S120P60F30,
    CLK_CFG_D120S120P30F60,
    CLK_CFG_D120S120P30F30,
    CLK_CFG_D80S80P40F40,
    CLK_CFG_D52S52P26F26,
    CLK_CFG_MAX,
    CLK_CFG_RAW = CLK_CFG_MAX,
};

enum {
    PLL_CFG_OFF = 0,
    PLL_CFG_320,
    PLL_CFG_480,
    PLL_CFG_320_480,
};

/*
 * cpu_sys pwr_ctrl_mode
 *  1) set by host idle task
 *  2) get by fw/host other task
 */
enum {
    CPU_SYS_POWER_ON_RESET  = 0x00,
    CPU_SYS_CLOCK_GATE      = 0x01,
    CPU_SYS_POWER_DOWN      = 0x02,
    CPU_SYS_HIBERNATE       = 0x03
};

/*
 * wifi_core sw_ctrl_mode
 *  1) set by fw/aon
 *  2) get by fw/aon
 */
enum {
    WIFI_CORE_CONTROLLED_BY_FW  = 0x00,
    WIFI_CORE_RELEASED_BY_FW    = 0x01,
    WIFI_CORE_CONTROLLED_BY_AON = 0x02,
    WIFI_CORE_RELEASED_BY_AON   = 0x03
};

/*
 * bt_core sw_ctrl_mode
 *  1) set by fw/aon
 *  2) get by fw/aon
 */
enum {
    BT_CORE_CONTROLLED_BY_FW    = 0x00,
    BT_CORE_RELEASED_BY_FW      = 0x01,
    BT_CORE_CONTROLLED_BY_AON   = 0x02,
    BT_CORE_RELEASED_BY_AON     = 0x03
};

enum {
    PWRCTRL_POWERUP = 0,
    PWRCTRL_POWERDOWN,
};

enum {
    VIO_SEL_INTERNAL        = 0,
    VIO_SEL_EXTERNAL        = 1,
};

/**
 * System initial clock configure
 */
#ifndef CONFIG_INITIAL_SYSTEM_CLOCK
#define CONFIG_INITIAL_SYSTEM_CLOCK     CLK_CFG_D240S240P120F60
#endif

/**
 * Chip VIO selection
 */
#ifndef CONFIG_VIO_SELECT
#define CONFIG_VIO_SELECT               VIO_SEL_INTERNAL
#endif

/**
 * Low power voltage configure
 */
#ifndef CONFIG_LP_CLOCK_LPO_512K_ENABLE
#define CONFIG_LP_CLOCK_LPO_512K_ENABLE         1
#endif
#ifndef CONFIG_LP_PMIC_VCORE_DCDC_DROP_ENABLE
#define CONFIG_LP_PMIC_VCORE_DCDC_DROP_ENABLE   1
#endif
#ifndef CONFIG_LP_PMIC_VCORE_LDO_DROP_ENABLE
#define CONFIG_LP_PMIC_VCORE_LDO_DROP_ENABLE    0
#endif
#ifndef CONFIG_LP_PMIC_VRTC_LDO_DROP_ENABLE
#define CONFIG_LP_PMIC_VRTC_LDO_DROP_ENABLE     1
#endif
#ifndef CONFIG_LP_PMIC_VRF_DCDC_DROP_ENABLE
#define CONFIG_LP_PMIC_VRF_DCDC_DROP_ENABLE     1
#endif
#ifndef CONFIG_LP_PMIC_VIO_SLP_PD_ENABLE
#define CONFIG_LP_PMIC_VIO_SLP_PD_ENABLE        1
#endif

/**
 * clock config table
 */
extern const unsigned int clk_cfg_code_tbl[CLK_CFG_MAX];

/**
 * get HCLK/PCLK/PER_CLK
 */
uint32_t sysctrl_clock_get(int sys_per);

/**
 * cfg DSP_CLK/SYS_CLK/PERI_CLK/FLSH_CLK
 */
void sysctrl_clock_cfg(int cfg);

/**
 * cfg FLSH_CLK
 */
void sysctrl_flash_clock_cfg(int cfg_flsh);

/**
 * cfg pll
 */
void sysctrl_pll_cfg(int cfg);

/**
 * sys reboot
 */
void sysctrl_chip_reboot(void);

/**
 * get bt power state
 */
int pwrctrl_bt_get(void);

/**
 * set bt power up/down
 */
void pwrctrl_bt_set(int ctrl);

/**
 * get wifi power state
 */
int pwrctrl_wifi_get(void);

/**
 * set wifi power up/down
 */
void pwrctrl_wifi_set(int ctrl);

/**
 * get mmsys power state
 */
int pwrctrl_mmsys_get(void);

/**
 * set mmsys power up/down
 */
void pwrctrl_mmsys_set(int ctrl);

/**
 * @brief   Update pwrctrl awake state
 */
void pwrctrl_awake_state_update(void);

/**
 * @brief   Get software record irq status
 * @return  Value of irq status
 */
uint32_t pwrctrl_sw_rec_irqstatus_get(void);

#endif /* _SYSCTRL_API_H_ */
