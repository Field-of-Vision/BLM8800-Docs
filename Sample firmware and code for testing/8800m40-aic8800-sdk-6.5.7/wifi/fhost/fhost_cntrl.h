...
void fhost_cntrl_cfgrwnx_set_fixed_rate(uint8_t sta_idx, union fixed_rate_ctrl_info *p_rate_info);
int fhost_cntrl_mm_set_filter(uint32_t value);
int fhost_cntrl_mm_set_rssi_req(uint8_t vif_idx, int8_t rssi);
int fhost_cntrl_mm_set_bssi_req(uint8_t vif_idx, uint8_t *bssi);
int fhost_cntrl_mm_set_channel_req(uint8_t ch_num);
