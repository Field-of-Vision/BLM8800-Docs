...
static USBH_StatusTypeDef USBH_MSC_InterfaceInit(USBH_HandleTypeDef *phost)
{
    ...
    else
    {
        ...
        #if 0
        phost->pActiveClass->pData = (MSC_HandleTypeDef *)USBH_malloc(sizeof(
                                         MSC_HandleTypeDef));
        #else
        phost->pActiveClass->pData = (MSC_HandleTypeDef *)&USB_MSC_Handle;
        #endif
...
{
    MSC_HandleTypeDef *MSC_Handle = (MSC_HandleTypeDef *)
            ...
            if (MSC_Handle->current_lun < MSC_Handle->max_lun)
            {
                ...
                switch (MSC_Handle->unit[MSC_Handle->current_lun].state)
                {
                    ...
                    case MSC_READ_INQUIRY:
                        scsi_status = USBH_MSC_SCSI_Inquiry(phost, MSC_Handle->current_lun,
                    ...
                    case MSC_READ_CAPACITY10:
                        scsi_status = USBH_MSC_SCSI_ReadCapacity(phost, MSC_Handle->current_lun,
                    ...
                    case MSC_REQUEST_SENSE:
                        scsi_status = USBH_MSC_SCSI_RequestSense(phost,  MSC_Handle->current_lun,
...
#if (USBH_USE_OS == 1)
                rtos_task_notify_setbits(phost->thread, USBH_OSEVENT_BIT(USBH_CLASS_EVENT), false);
#endif
            }
            else
            {
        ...
        uint8_t lun)
{
    ...
    switch (MSC_Handle->unit[lun].state)
    {
        ...
        case MSC_REQUEST_SENSE:
            scsi_status = USBH_MSC_SCSI_RequestSense(phost, lun,
                          &MSC_Handle->unit[lun].sense);
                                 ...
                                 uint32_t length)
{
    ...
    {
        if (((phost->Timer - timeout) > (10000 * length))
