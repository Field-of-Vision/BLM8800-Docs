...
python ../../../tools/scons.py -u . PRODUCT=host-wifi PLF=aic8800 BT=armgcc_4_8 BUILD_CMD="$0 $*" SOFTAP=on ROM_VER=3 \
        TGT_MODE=wifi_audio WIFI_AUD_MEM=on\
        FHOST_APP=interphone UTILS=on BAND5G=on PMIC_MCLK=on PMIC_VER=aud \
        ASIO=on WIFI_AUDIO=on AUDIO=on HEAP_SIZE=0x1A000 \
        TGTNAME=host_wb_interphone_ap WIFI_VOICE_ROLE=dongle $opt
