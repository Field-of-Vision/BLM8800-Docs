...
#endif
#define MSADC_CURR_DIVISOR          76.8
...
{
    float vout0, vout1;
    wlan_rf_wf_pa_isense_en_setf(1);
    wlan_rf_wf_pa_isense_icsel_bit_setf(0);
    // TODO: sleep 100us
    vout0 = msadc_volt_sense(12, 2);
    wlan_rf_wf_pa_isense_en_setf(1);
    wlan_rf_wf_pa_isense_icsel_bit_setf(1);
    // TODO: sleep 100us
    vout1 = msadc_volt_sense(12, 2);
    wlan_rf_wf_pa_isense_en_setf(0);
...
void msadc_input_sel_avdd33(void)
{
    ...
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val | MSADC_REG_CFG_MSADC_SW_MUX_BITS(14);
    MSADC_DEBUG_PRINTF("wr       : [%p] = %08x\n", (&AIC_MSADC->cfg_msadc_sw_ctrl1), (reg_val | MSADC_REG_CFG_MSADC_SW_MUX_BITS(14)));
    msadc_set_diff_mode();
...
void msadc_input_sel_vcore(void)
{
    ...
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val | (MSADC_REG_CFG_MSADC_SW_MUX_BITS(10) | 0);
    MSADC_DEBUG_PRINTF("wr       : [%p] = %08x\n", (&AIC_MSADC->cfg_msadc_sw_ctrl1), (MSADC_REG_CFG_MSADC_SW_MUX_BITS(10) | 0));
...
void msadc_input_sel_pa_sense(void)
{
    uint32_t reg_val;
    reg_val = AIC_MSADC->cfg_msadc_sw_ctrl1 & ~MSADC_REG_CFG_MSADC_SW_MUX_BITS_MASK;
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val | MSADC_REG_CFG_MSADC_SW_MUX_BITS(12);
...
void msadc_input_sel_tsm1(void)
{
...
#if 0
void msadc_pa_cal_sel5u(void)
{
    wlan_rf_wf_pa_isense_en_setf(1);
    wlan_rf_wf_pa_isense_icsel_bit_setf(0);
}
void msadc_pa_cal_sel_core(void)
{
    wlan_rf_wf_pa_isense_en_setf(1);
    wlan_rf_wf_pa_isense_icsel_bit_setf(1);
}
void msadc_pa_cal_off(void)
{
    wlan_rf_wf_pa_isense_en_setf(0);
}
#endif
...
float msadc_ms_temp_sense1(void)
{
    ...
    msadc_ad_config();
    msadc_set_diff_mode();
    msadc_input_sel_tsm1();
    ms_val = msadc_ms();
    return (ms_val / (float)MSADC_DENOM_REC - 1) * 596 - 260;
}
...
void msadc_cal_avdd13(void)
{
    ...
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val |  (MSADC_REG_CFG_MSADC_SW_MUX_BITS(14) | // mux
...
void msadc_cal_vcore(void)
{
    ...
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val |  (MSADC_REG_CFG_MSADC_WINDOW(512) |
                                                MSADC_REG_CFG_MSADC_SW_MUX_BITS(10) | 0);
...
{
    // usb dm test mode
    AIC_MMSYSCTRL->usb_cfg0 |= (
        (0x01UL << 28) |
        (0x01UL << 26));
    AIC_MMSYSCTRL->usb_cfg2 |= (
        (0x01UL << 25) |
        (0x01UL << 24) |
        (0x01UL << 17) |
        (0x01UL << 16) |
    ...
    AIC_ANAREG1->ana_port_ctrl |= ANALOG_REG1_RF_CFG_ANA_TEST_ENABLE_USB;

    // msadc cfg
    unsigned int ana_ctrl0, sw_ctrl1;
    ana_ctrl0 = AIC_MSADC->cfg_msadc_ana_ctrl0 &
        ~(MSADC_REG_CFG_ANA_MSADC_CHNP_SEL |
          MSADC_REG_CFG_ANA_MSADC_CMP_BIT |
          MSADC_REG_CFG_ANA_MSADC_VBE_CAL_BIT_MASK |
          MSADC_REG_CFG_ANA_MSADC_SDM_MODE |
          ...
          MSADC_REG_CFG_ANA_MSADC_DITHER_MODE_MASK |
          0);
    sw_ctrl1 = AIC_MSADC->cfg_msadc_sw_ctrl1 &
        ~(MSADC_REG_CFG_MSADC_SW_MUX_BITS_MASK |
          MSADC_REG_CFG_MSADC_SW_DIFF_MODE);
    ana_ctrl0 |=
        (0 | // chap_sel
         MSADC_REG_CFG_ANA_MSADC_CMP_BIT |
         MSADC_REG_CFG_ANA_MSADC_VBE_CAL_BIT(0x79) | // vbe_cal_bit
         MSADC_REG_CFG_ANA_MSADC_SDM_MODE | // sdm_mode
         MSADC_REG_CFG_ANA_MSADC_IBIAS_BIT(0x2) | // ibias_bit
         MSADC_REG_CFG_ANA_MSADC_ADC_FF_EN | // adc_ff_en
         0 | // ts_mode
         MSADC_REG_CFG_ANA_MSADC_VCOM_BIT(0x1) | // vcom_bit
         MSADC_REG_CFG_ANA_MSADC_DITHER_MODE(0x1) | // dither_mode
         0);
    sw_ctrl1 |=
        (MSADC_REG_CFG_MSADC_SW_MUX_BITS(0xB) |
         0 | // diff_mode
         0);
    AIC_MSADC->cfg_msadc_ana_ctrl0 = ana_ctrl0;
    AIC_MSADC->cfg_msadc_sw_ctrl1 = sw_ctrl1;
    #if 0
    // start
    AIC_MSADC->cfg_msadc_sw_ctrl0 = MSADC_REG_CFG_MSADC_SW_START_PULSE;
    ...
    #else
    int flag = 0;
    uint32_t idac_bit_end;
    uint32_t ms_val;
    int dm_mv, prev_dm_mv = 0;
    ms_val = msadc_ms();
    dm_mv = (int)ms_val * 1214 / MSADC_DENOM_REC - 1214; // Convert to mV
    #endif
    uint32_t reg_val = AIC_MMSYSCTRL->usb_ana_cfg0;
    uint32_t idac_bit = (reg_val & MM_SYS_CTRL_RG_USB_HS_IDAC_BIT_MASK) >> MM_SYS_CTRL_RG_USB_HS_IDAC_BIT_LSB;;
    dbg("USB dm init idac_bit:%x, ms=%x, v:%dmV\n", idac_bit, ms_val, dm_mv);

    if (dm_mv > CAL_USB_DM_TARGET) {
        flag = 1;
        idac_bit_end = 0xF;
    } else if (dm_mv < CAL_USB_DM_TARGET) {
        flag = -1;
        idac_bit_end = 0x0;
    }
    if (flag) {
        int err_detect = 0;
        int idac_bit_sel = 0;
        while (((dm_mv - CAL_USB_DM_TARGET) * flag > 0) && (((int)idac_bit - (int)idac_bit_end) * flag < 0)) {
            prev_dm_mv = dm_mv;
            idac_bit += flag;
            AIC_MMSYSCTRL->usb_ana_cfg0 = (reg_val & ~MM_SYS_CTRL_RG_USB_HS_IDAC_BIT_MASK) |
                ((idac_bit << MM_SYS_CTRL_RG_USB_HS_IDAC_BIT_LSB) & MM_SYS_CTRL_RG_USB_HS_IDAC_BIT_MASK);
            ms_val = msadc_ms();
            dm_mv = (int)ms_val * 1214 / MSADC_DENOM_REC - 1214; // Convert to mV
            dbg("USB dm set idac_bit:%x, ms=%x, v:%dmV\n", idac_bit, ms_val, dm_mv);
            if ((prev_dm_mv - dm_mv) * flag < 0) {
                err_detect = 1;
                break;
            }
        }
        if ((CAL_USB_IDAC_BIT_LOW > idac_bit) || (idac_bit > CAL_USB_IDAC_BIT_HIGH)) {
            err_detect = 2;
        }
        if (err_detect) {
            idac_bit = CAL_USB_IDAC_BIT_DEFAULT;
            idac_bit_sel = 2;
            dbg("WARN %d\n", err_detect);
        } else {
            if ((dm_mv < CAL_USB_DM_TARGET) && (CAL_USB_DM_TARGET < prev_dm_mv)) {
                if ((prev_dm_mv - CAL_USB_DM_TARGET) < (CAL_USB_DM_TARGET - dm_mv)) {
                    idac_bit_sel = 1;
                }
            }
            if ((dm_mv > CAL_USB_DM_TARGET) && (CAL_USB_DM_TARGET > prev_dm_mv)) {
        ...
        if (idac_bit_sel) {
            ...
            ms_val = msadc_ms();
            dm_mv = (int)ms_val * 1214 / MSADC_DENOM_REC - 1214; // Convert to mV
            dbg("USB dm sel idac_bit:%x, ms=%x, v:%dmV\n", idac_bit, ms_val, dm_mv);
        }
    }

    // usb dm test mode recover
    AIC_MMSYSCTRL->usb_cfg0 &= ~(
        (0x01UL << 28) |
        (0x01UL << 26));
    AIC_MMSYSCTRL->usb_cfg2 &= ~(
...
int msadc_temp_measure(int tst)
{
    ...
    if (tst == MSADC_TEMP_CHIP) {
        ...
        msadc_set_diff_mode();
        msadc_input_sel_tsm1();
        // ms_mV = (ms_val / (float)MSADC_DENOM_REC - 1) * 596 - 260;
        ms_val = msadc_ms();
        ret = (int)ms_val * 596 / MSADC_DENOM_REC - 596 - 260;
    }
    return ret;
}
