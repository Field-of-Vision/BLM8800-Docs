...
__INLINE void uart1_dbufcfg_pack(uint8_t txdrst, uint8_t rxdrst, uint8_t dbufen)
{
    REG_PL_WR(UART1_DBUFCFG_ADDR, ((uint32_t)txdrst << 2) | ((uint32_t)rxdrst << 1) | ((uint32_t)dbufen << 0));
}
